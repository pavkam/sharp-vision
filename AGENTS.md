...
- Avoid repeated prefixes and suffixes across related members. In particular, do
  not name an `EventArgs` type after its raising type's full name for
  disambiguation alone. `TabSelectionChangedEventArgs`,
  `ProgressValueChangedEventArgs`, and `ExpandedChangedEventArgs` are correct as
  written — a short, property-focused name reads better than
  `{FullOwningTypeName}{Property}ChangedEventArgs`, and an `EventArgs` type
  stays a top-level type in its own file either way, never nested inside the
  control it describes.
- The three related member kinds around one boolean concept are named by kind,
  not by bare adjective, and never share a name with each other: a method that
  performs the transition is a verb (`Open()`), the event raised after the
  transition commits is the past participle (`Opened`), and the boolean state
  property is `Is`-prefixed (`IsOpen`, `IsEnabled`, `IsChecked`, `IsSelected`,
  `IsExpanded`, `IsReadOnly`). This holds even where only one or two of the
  three exist for a given concept — a boolean state property is `Is`-prefixed on
  its own, not only when disambiguating against a sibling event or method. A
  verb-phrase or modal-permission name that already reads as a predicate without
  "Is" is not itself a bare adjective and stays as written — `AllowNull`,
  `ShowHeader`, `UseMnemonic`, `ContainsFocus`, `AcceptsReturn`, `ClipToBounds`
  — matching the same convention WinForms and WPF use for these shapes. When
  renaming an existing bare-adjective boolean property, rename every reference
  across production code, tests, examples, and docs in the same change, and
  verify with a full solution build (a missed reference fails to compile, it
  does not silently keep the old name).
...
- Never reference a GitHub issue, pull request, or discussion number (`#NN`
  patterns, `issue #NN`, `fixes #NN`, PR URLs, etc.) in code, comments, or XML
  documentation. Explain the _why_ directly and self-containedly instead; the
  reasoning must stand on its own without an external tracker lookup. This
  repository's issue tracker is not a durable citation target: numbers get
  renumbered, issues get deleted, and a comment that only makes sense next to an
  issue number is incomplete documentation.
...
- Any change to a page under `docs/controls/**` first loads the
  [control-page contract and template](docs/documentation-guide.md#control-page-template)
  and the `ui-components` skill, inspects the current declaration, XML
  documentation, tests, and Showcase pane, and follows the exact template.
  Before the change is done, run `npm run lint:doc-content`,
  `npm run lint:markdown`, `npm run lint:links`, `npm run test:docs`, and
  `make lint`.
...
- Never reference a GitHub issue, pull request, or discussion number in `docs/`
  content, for the same reason code and comments avoid it: see the rule under
  "C# and API rules". Describe the rationale and the current behavior directly.
