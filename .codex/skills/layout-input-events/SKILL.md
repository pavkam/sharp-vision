...
description:
  Use when changing SharpVision measure or arrange,
  fixed/percentage/auto/proportional sizing, margin, padding, alignment,
  scrolling, routed input, focus, pointer capture, dispatcher affinity, resize,
  idle, timers, or lifecycle event order.
---
...
1. If the task selects panels or composes a user-facing application surface, use
   `designing-user-interfaces`. This skill owns layout engine behavior and event
   semantics, not visual design by itself.
2. Read `docs/concepts/layout.md`, `docs/concepts/scrolling.md`,
   `docs/concepts/input-routing.md`, `docs/concepts/focus.md`,
   `docs/concepts/threading.md`, and `docs/concepts/lifecycle-events.md`.
3. State units, box-model edges, percentage base, rounding, event order, and
   invalidation effects before changing APIs.
4. Write failing tests with recording controls plus fake terminal, dispatcher,
   and clock. Assert order and committed geometry, not merely final values.
5. Implement in dependency order: sizing primitives, measure/arrange, panels,
   scroll host, routed input, then dispatcher/runtime events.
6. Reject reentrant layout. Queue invalidation and coalesce resize records on
   the dispatcher.
7. Update concept docs, control contracts, diagrams, and showcase resize/scroll
...
- Treating Canvas coordinates or repeated fixed widths as application layout
  when Grid, Dock, Stack, or Overlay expresses the relationship directly.
