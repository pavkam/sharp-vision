---
name: figlet-fonts
description: Use when changing SharpVision FIGfont parsing, FIGlet fitting or smushing, FigletFont, FigletCatalog, FigletText, embedded .flf/.tlf resources, font provenance, the 400-font ZIP, or its audit and packaging scripts.
---
...
1. Read
   `docs/superpowers/specs/2026-07-11-figlet-catalog-design.md`,
   `data/figlet/README.md`, and the nearest tests under
   `tests/SharpVision.Tests/Fonts/`.
...
`upstream-unverified`; those values are release blockers, not permission
grants. Never relabel, omit, or strip notices to make the audit green.
...
node scripts/audit-figlet-fonts.mjs --source "$FIGLET_FONT_SOURCE" \
  ...
  --output data/figlet/fonts.manifest.json --check
