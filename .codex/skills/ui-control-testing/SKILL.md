...
The coverage system enforces that every control has exactly one of each exclusive
pair (Hover/HoverExcluded, Focus/FocusExcluded, Tab/TabExcluded,
...
Verify that a focused child handles keys first, and unhandled keys bubble to
the parent (e.g., an AutoScroll container):
...
- Forgetting `Overflow = Overflow.Clip` on `ControlText` in surface tests —
  text wraps unexpectedly without it.
- Not setting `HorizontalAlignment = Stretch` on showcase wrappers for
  ...
  percentage.
- Using `_ = ScrollBy(...)` instead of checking the return value — swallows
  the key event even when no scrolling occurred.
