...
description: Use when changing SharpVision grapheme segmentation, Rune decoding, Unicode width, emoji, combining marks, wide cells, wrapping, clipping, cursor placement, selection, or cell-coordinate behavior.
---
...
Measure terminal cells by extended grapheme cluster, not `char` and not a sum
of Rune widths. Keep every consumer on one versioned geometry implementation.
