...
const embeddedResourcePaths = new Set([
    path.join("src", "SharpVision", "Fonts", "Resources", "fonts.zip"),
]);
...
export const validateExtern = async (root) => {
    ...
    const misplaced = files.find(
        (file) =>
            isResource(file) &&
            !file.startsWith(`extern${path.sep}`) &&
            !embeddedResourcePaths.has(file),
    );
    ...
    for (const file of files.filter((value) =>
        referenceExtensions.has(path.extname(value)))) {
...
const main = async () => {
    ...
    process.stdout.write(
        "External resources are documented and stored at audited paths.\n",
    );
};
