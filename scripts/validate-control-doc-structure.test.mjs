...
const fixtureSnapshot = `﻿namespace Fixture.Controls
{
    ...
    public abstract class Bucket<TItem>
        where TItem : Fixture.Controls.ControlBase
    {
        protected Bucket() { }
        protected virtual void OnFilled(TItem item) { }
    }
}
`;
...
async function buildFixtureRoot(pageText) {
    ...
    await mkdir(
        join(root, "tests", "SharpVision.Compatibility.Tests", "Snapshots"),
        {
            recursive: true,
        },
    );
...
test("deriveDocumentedType_WhenSlugIsOverridden_ReturnsTheOverrideKey", () => {
    ...
    assert.equal(
        deriveDocumentedType("composite-control"),
        "CompositeControlBase#0",
    );
...
test("splitTopLevelCommas_WhenTextHasNestedGenerics_SplitsOnlyTopLevelCommas", () => {
    assert.deepEqual(splitTopLevelCommas("Foo<Bar,Baz>, Qux"), [
        "Foo<Bar,Baz>",
        "Qux",
    ]);
...
test("parseSnapshotTypes_WhenGivenTheFixtureSnapshot_MapsEveryTypeToItsBaseAndMembers", () => {
    ...
    assert.ok(map.get("Bucket#1").members.has("OnFilled"));
...
test("extractInheritanceEdges_WhenBodyMixesEdgeKinds_ExtractsOnlyInheritanceEdges", () => {
    ...
    assert.deepEqual(extractInheritanceEdges(body), [
        { parent: "A", child: "B" },
    ]);
});
...
test("extractCheckableTokens_WhenCellIsAnInheritedMember_ReturnsThePlainToken", () => {
    assert.deepEqual(extractCheckableTokens("Inherited `Text`"), [
        { name: "Text" },
    ]);
});
...
test("extractCheckableTokens_WhenCellIsAnInheritedGroupedList_ReturnsOneTokenPerSpan", () => {
    assert.deepEqual(
        extractCheckableTokens("Inherited `Command`, `CommandParameter`"),
        [{ name: "Command" }, { name: "CommandParameter" }],
    );
});

test("extractCheckableTokens_WhenCellIsAnInheritedMethod_StripsParens", () => {
    assert.deepEqual(extractCheckableTokens("Inherited `ResetBorder()`"), [
        { name: "ResetBorder" },
    ]);
});

test("extractCheckableTokens_WhenCellIsAGroupedList_ReturnsOneTokenPerSpan", () => {
    assert.deepEqual(extractCheckableTokens("`Width`, `Height`"), [
        { name: "Width" },
        { name: "Height" },
    ]);
});
...
test("extractCheckableTokens_WhenCellIsAMultiParamMethod_StripsParensWithoutSplittingOnInnerCommas", () => {
    assert.deepEqual(extractCheckableTokens("`ScrollBy(int x, int y)`"), [
        { name: "ScrollBy" },
    ]);
});

test("extractCheckableTokens_WhenCellIsAnAttachedProperty_ReturnsAnOwnerPropertyToken", () => {
    assert.deepEqual(extractCheckableTokens("`Dock.Side`"), [
        { owner: "Dock", property: "Side" },
    ]);
});
...
test("validateTierB_WhenALeadParagraphNamesAGenericSnapshotType_ChecksAgainstThatArity", () => {
    const map = parseSnapshotTypes(fixtureSnapshot);
    const lines = [
        "## API",
        "",
        ...
        "### Typed buckets",
        "",
        "`Bucket<TItem>` exposes one hook a derived bucket overrides.",
        "",
        "| Member            | Type   | Default | Description |",
        "| ----------------- | ------ | ------- | ----------- |",
        "| `OnFilled(TItem)` | `void` | No-op   | Real.       |",
        "| `Bogus`           | `int`  | `0`     | Not real.   |",
    ];
    const headings = extractSubsectionHeadings(lines);
    ...
    assert.equal(result.errors.length, 1);
    assert.match(result.errors[0], /member `Bogus` was not found on Bucket/);
});
...
test("validateTierB_WhenAnInheritedCellNamesARealOrFakeAncestorMember_PassesOrFailsAccordingly", () => {
    ...
    assert.match(
        result.errors[0],
        /member `FakeMember` was not found on Widget/,
    );
});
...
test("validateInheritanceSection_WhenTheFenceIsMissing_ReportsAViolation", () => {
    ...
    assert.match(
        validateInheritanceSection(lines, headings).error,
...
test("validateControlDocStructure_WhenASecondaryTypeTableHasAFabricatedMember_CatchesItEndToEnd", async () => {
    ...
    try {
        ...
        assert.match(
            result.errors[0],
            /docs\/controls\/widget\.md: member `Bogus` was not found on Gadget/,
        );
    } finally {
        await rm(root, { recursive: true, force: true });
    }
...
test("validateControlDocStructure_WhenTheH2SpineIsOutOfOrder_ReportsASpineViolation", async () => {
    ...
    const beforeInheritance = validWidgetPage.slice(
        0,
    ...
    const apiChunk = validWidgetPage.slice(
        validWidgetPage.indexOf("## API"),
        validWidgetPage.indexOf("## Example"),
    );
    ...
    try {
        ...
        assert.match(
            result.errors[0],
            /docs\/controls\/widget\.md: H2 spine violation/,
        );
    } finally {
        await rm(root, { recursive: true, force: true });
    }
...
test("validateControlDocStructure_WhenKeyboardSectionIsMissing_ReportsAViolation", async () => {
    ...
    try {
        ...
        assert.match(
            result.errors[0],
            /docs\/controls\/widget\.md: .*## Keyboard is required after ## API/,
        );
    } finally {
        await rm(root, { recursive: true, force: true });
    }
...
test("validateControlDocStructure_WhenKeyboardTableHeaderIsWrong_ReportsAViolation", async () => {
    ...
    try {
        ...
        assert.match(
            result.errors[0],
            /## Keyboard table header is .*expected "\| Key \| Behavior \|"/,
        );
    } finally {
        await rm(root, { recursive: true, force: true });
    }
...
test("validateControlDocStructure_WhenKeyboardTableHasNoBehaviorRows_ReportsAViolation", async () => {
    ...
    try {
        ...
        assert.match(
            result.errors[0],
            /## Keyboard table has no behavior rows/,
        );
    } finally {
        await rm(root, { recursive: true, force: true });
    }
...
test("validateControlDocStructure_WhenInheritanceHasNoMermaidFence_ReportsAViolation", async () => {
    ...
    try {
        ...
        assert.match(
            result.errors[0],
            /## Inheritance has no fenced code block/,
        );
    } finally {
        await rm(root, { recursive: true, force: true });
    }
...
test("validateControlDocStructure_WhenAPageIsNamedIndex_IsExempt", async () => {
    ...
    try {
        ...
        await mkdir(join(root, "docs", "controls"), { recursive: true });
        await writeFile(
            join(root, "docs", "controls", "index.md"),
            "# Controls\n\nJust a catalog.\n",
        );
...
test("validateControlDocStructure_WhenAPageIsPendingMigration_IsSkippedWithAWarning", async () => {
    ...
    try {
        ...
        await writeFile(
            join(root, "docs", "controls", "pressable.md"),
            "# Pressable\n\nNot migrated yet.\n",
        );
        ...
        assert.match(
            result.warnings[0],
            /docs\/controls\/pressable\.md: PENDING_MIGRATION/,
        );
    } finally {
