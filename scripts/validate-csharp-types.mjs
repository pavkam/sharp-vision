...
function braceDepthAt(content, index) {
  let depth = 0;
...
function isTopLevelType(content, index) {
  const usesFileScopedNamespace = /\bnamespace\b[^{;]*;/m.test(content);
  const maxDepth = usesFileScopedNamespace ? 0 : 1;
  return braceDepthAt(content, index) <= maxDepth;
}
...
function collectTypes(content) {
  ...
  for (const match of visible.matchAll(recordType)) {
    if (isTopLevelType(visible, match.index)) {
  ...
  for (const match of visible.matchAll(ordinaryType)) {
    const precedingCharacter = visible.slice(0, match.index).trimEnd().at(-1);
...
function collectPrimaryConstructors(content) {
  ...
  return [...visible.matchAll(primaryConstructorType)]
    .filter((match) => isTopLevelType(visible, match.index))
    .map((match) => match[1]);
}
