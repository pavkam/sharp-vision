...
internal readonly struct DocumentFlowToken
{
    ...
    public static DocumentFlowToken ForText(
        ...
        DocumentFaceKind face,
        int linkIndex) =>
        new(kind, parsedRunIndex, offset, length, cells, default, null, face, linkIndex);
    ...
    /// <returns>The token.</returns>
    [Pure]
    public static DocumentFlowToken ForBlank(int cells, DocumentFaceKind face, int linkIndex) =>
        new(DocumentFlowTokenKind.Space, -1, 0, 0, cells, new Rune(' '), null, face, linkIndex);
    ...
    public static DocumentFlowToken ForControl(ControlBase control, int cells) =>
        new(DocumentFlowTokenKind.Control, -1, 0, 0, cells, default, control, DocumentFaceKind.Body, -1);

    /// <summary>Creates a hard line break token.</summary>
    /// <returns>The token.</returns>
    [Pure]
    public static DocumentFlowToken ForBreak() =>
        new(DocumentFlowTokenKind.Break, -1, 0, 0, 0, default, null, DocumentFaceKind.Body, -1);
}
