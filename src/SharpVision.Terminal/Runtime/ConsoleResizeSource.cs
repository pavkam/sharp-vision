...
public sealed class ConsoleResizeSource: IResizeSource
{
    ...
    public async ValueTask<Dimensions> ReadAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (true)
            {
                ...
                // as a real resize would publish a spurious suspend to the sink.
    ...
    public bool TryReadCurrent(out Dimensions value)
    {
        ...
        {
            // Recording this as the last-published size prevents the immediately following
            // ReadAsync from treating an unchanged size as a new resize and republishing a
            // duplicate event right after startup's initial snapshot.
            _last = current;
