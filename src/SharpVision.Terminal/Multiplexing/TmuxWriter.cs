...
/// </remarks>
[PublicAPI]
public static class TmuxWriter
{
    ...
    /// <summary>Unwraps one complete tmux DCS envelope without exposing a partial payload.</summary>
    /// <param name="envelope">The complete borrowed <c>DCS tmux; ... ST</c> envelope.</param>
    /// <param name="destination">The non-null synchronously consumed byte destination.</param>
    /// <returns>True when the complete envelope and every ESC pair are valid.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="destination"/> is null.</exception>
    public static bool TryUnwrapEnvelope(
        ReadOnlySpan<byte> envelope,
        IBufferWriter<byte> destination)
    {
        ArgumentNullException.ThrowIfNull(destination);

        return envelope.StartsWith("\u001bP"u8) &&
               envelope.EndsWith(Terminator) &&
               TryUnwrap(envelope[2..^Terminator.Length], destination);
    }
}
