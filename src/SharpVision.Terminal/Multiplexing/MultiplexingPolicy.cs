...
public sealed class MultiplexingPolicy
{
        ...
        int maxEnvelopeBytes = 1_048_576)
    {
        ...
        ArgumentOutOfRangeException.ThrowIfNotDefined(passthrough, nameof(passthrough), "The passthrough mode is unknown.");

        ArgumentOutOfRangeException.ThrowIfUndefinedFlags(approvedOperations, _allOperations, nameof(approvedOperations), "The operation set contains an unknown family.");
    ...
    public bool Active => _layers.Count > 0 &&
    ...
    public static MultiplexingPolicy Detect(IReadOnlyDictionary<string, string?> environment)
    {
        ...
        var kind = environment.TryGetValue(EvidenceEnvironmentVars.Tmux, out var tmux) && !string.IsNullOrWhiteSpace(tmux)
            ...
            : StartsWith(term, "tmux-")
                ? MultiplexerKind.Tmux
                : environment.TryGetValue(EvidenceEnvironmentVars.Sty, out var sty) && !string.IsNullOrWhiteSpace(sty)
