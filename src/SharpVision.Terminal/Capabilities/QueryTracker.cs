...
public sealed class QueryTracker
{
    ...
    internal QueryMatch Match(in StatusResponse response, DateTimeOffset now)
    {
        ...
        _ = ExpireCore(now);
        PruneHistory(now);
        return !response.Valid || response.Name == StatusName.Unknown
            ? Unknown()
            : MatchCore(
    ...
    internal QueryMatch Match(CapabilityResponse response, DateTimeOffset now)
    {
        ...
        // A failed response may echo the single unsupported capability name it was asked about;
        // that still identifies which pending query it answers, so it retires the
        // query the same way a successful single-item response does. A failure with no echoed
        // name (or any response naming more than one capability) cannot be attributed to one
        // query and stays Unknown.
    ...
    {
        ArgumentOutOfRangeException.ThrowIfNotDefined(kind, nameof(kind), "The query kind is unknown.");
    ...
    private static void Validate(CapabilityName name) =>
        ArgumentOutOfRangeException.ThrowIfNotDefined(name, nameof(name), "The capability name is undefined.");
