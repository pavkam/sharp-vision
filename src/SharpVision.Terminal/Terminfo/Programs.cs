...
internal sealed class Programs
{
    ...
    public bool Has(string name)
    {
        ...
        if (!_values.TryGetValue(name, out var program) ||
            (!program.IsIntrinsic && !program.IsCompiled))
        {
            return false;
        }
        ...
        IBufferWriter<byte> destination)
    {
        ...
        return executable &&
            (program.IsCompiled
                ...
                : program.IsIntrinsic && TryWriteIntrinsic(name, parameters, destination));
    }
    ...
    public bool SemanticallyEquals(Programs other)
    {
        ...
        foreach (var pair in _values)
        {
            if (!other._values.TryGetValue(pair.Key, out var candidate) ||
                pair.Value.IsIntrinsic != candidate.IsIntrinsic ||
        ...
        out ReadOnlyMemory<byte> disable)
    {
        ...
        try
        {
            if (enableProgram.IsCompiled && disableProgram.IsCompiled)
            {
            ...
            else if (enableProgram.IsIntrinsic && disableProgram.IsIntrinsic)
            {
    ...
    private bool ClassifyZeroParameterPair(string enableName, string disableName)
    {
        if (!_values.TryGetValue(enableName, out var enable) ||
            ...
            (!enable.IsIntrinsic && enable.ParameterCount != 0) ||
            (!disable.IsIntrinsic && disable.ParameterCount != 0))
        {
            return false;
        }

        if (enable.IsIntrinsic || disable.IsIntrinsic)
        {
            return enable.IsIntrinsic &&
                disable.IsIntrinsic &&
                IntrinsicSupports(enableName) &&
                IntrinsicSupports(disableName);
        }

        if (!enable.IsCompiled || !disable.IsCompiled)
        {
            return false;
        }
