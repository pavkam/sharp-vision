...
internal sealed class NonRetainedGraphicsBackend: IGraphicsBackend
{
        ...
        GraphicsContext context = default)
    {
        ...
        var enableSixel = _enableSixel && (context.Profile is null || GraphicsBackendSelector.Authoritative(
        ...
        try
        {
            for (var index = 0; index < back.PlacementCount; index++)
            {
                ...
                // earlier placements in this same frame already consumed.
    ...
    // degrade path the full-budget pre-check couldn't provide on its own. PNG decode
        ...
        int maxOutputBytes)
    {
        ...
        catch (InvalidOperationException)
        {
            ...
            // Prepare and hang a hosted Application.
            return false;
        }
    }
        ...
        int maxOutputBytes)
    {
        ...
        if (_route is null)
        {
            // Encode into a scratch buffer first: if SixelWriter.Write fails partway (budget
            // exceeded or malformed source), nothing must reach the shared output buffer, or the
            // cursor move written for this placement would be emitted with no matching sixel
            // data behind it.
        ...
        Span<ImageSource?> itermImages)
    {
        ...
        for (var index = 0; index < frame.PlacementCount; index++)
        {
            ...
            {
                // Keep protocol authorization, image representation, and placement semantics
                // distinct so every cell fallback is observable without a misleading reason.
