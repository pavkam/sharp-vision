...
internal sealed class ActiveQueryDiscoveryStrategy
{
        ...
        string? describedTerminalName = null)
    {
        ...
        // Only a route approved for capability queries can carry an APC probe across a
        // multiplexer. A detected but unroutable multiplexer consumes it instead, whether there
        // is no route at all or the route is merely present but not approved for capability
        // queries: tmux parses a bare APC string as a pane title, so the probe would overwrite
        // the user's pane title and could never be answered. Skipping it also avoids spending a
        // query deadline on a reply that cannot arrive.
        var canDeliverApc = _usesExplicitOuterProfile || _options.Multiplexing.Layers.Count == 0;
