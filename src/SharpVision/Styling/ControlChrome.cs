...
internal static class ControlChrome
{
    ...
    extension(ControlBase control)
    {
            ...
            ChromeRenderOptions? options = null)
        {
            ...
            if (appearance.Shadow.Visible && !settings.SkipShadow)
            {
                canvas.DrawShadow(
                    control,
                    body,
            ...
            if (background == BackgroundMode.Opaque && !settings.SkipBodyFill &&
                (!settings.ClearBodyWhenPressedWithShadow || !isPressed || !appearance.Shadow.Visible))
            {
                canvas.Clear(FillBounds(body, appearance.Border.Sides), bodyStyle);
            }
            else if (settings.ClearBodyWhenPressedWithShadow && isPressed && appearance.Shadow.Visible)
            {
                canvas.Clear(FillBounds(body, appearance.Border.Sides), bodyStyle);
            }
        }
