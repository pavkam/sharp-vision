...
internal static class ControlGlyphs
{
    ...
    /// <summary>Gets loading and failed status indicator glyphs.</summary>
    public static StatusGlyphs Status { get; } = new(
        Glyph('•', '*'), Glyph('✗', 'x'));

    /// <summary>Gets month-navigation arrow glyphs.</summary>
    public static CalendarGlyphs Calendar { get; } = new(
        Glyph('<', '<'), Glyph('>', '>'));

    /// <summary>Gets the ColorPlane currently-selected-coordinate marker glyph.</summary>
    public static ColorPlaneGlyphs ColorPlane { get; } = new(Glyph('◆', '*'));
