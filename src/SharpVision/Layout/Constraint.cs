...
public readonly record struct Constraint
{
    ...
    public bool WidthBounded => Width.HasValue;

    /// <summary>Gets whether height has a finite cell bound, including zero.</summary>
    public bool HeightBounded => Height.HasValue;

    /// <inheritdoc />
    public override string ToString()
    {
        var w = Width.HasValue ? Width.Value.ToString(CultureInfo.InvariantCulture) : "∞";
        var h = Height.HasValue ? Height.Value.ToString(CultureInfo.InvariantCulture) : "∞";
        return $"{w}×{h}";
    }
}
