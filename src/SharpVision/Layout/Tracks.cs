...
public static class Tracks
{
    ...
    /// priority and lose their shared cumulative rounding edges. A caller that allocates within
    /// an intentionally unbounded pool - <paramref name="available"/> null - supplies the
    /// visible viewport here instead of the growing extent it allocates within, so a scrolling
    /// panel's Percent tracks resolve against what is actually visible rather than an extent
    /// that is, by construction, too small to have ever accounted for their own true size.
    /// </param>
    /// <exception cref="ArgumentOutOfRangeException">
        ...
        int? percentBase)
    {
        ...
        {
            // An unbounded axis has no ceiling, so nothing competes for space: every track
            // simply gets its own request. Percent still resolves against percentBase when a
            // caller supplies one - an armed panel handing this axis an intentionally unbounded
            // pool so every other track keeps its full, non-competing size, while still wanting
            // Percent tracks sized against the visible viewport rather than falling back to
            // their own unrelated intrinsic request. A caller with no percentBase
            // keeps the prior behavior exactly: Percent, like Star, reports its own automatic
            // request, since neither has a defined size without a bounded axis to divide.
            var unboundedCumulativePercent = 0d;
            var unboundedPreviousPercentEdge = 0;
            ...
            for (var index = 0; index < lengths.Length; index++)
            {
                var requested = lengths[index].Kind switch
                {
                    ...
                    LengthKind.Percent when percentBase.HasValue => ResolvePercentRequest(
                        percentBase.Value,
                        lengths[index].Value,
                        ref unboundedCumulativePercent,
                        ref unboundedPreviousPercentEdge),
                    LengthKind.Auto or LengthKind.Percent or LengthKind.Star => automatic[index],
