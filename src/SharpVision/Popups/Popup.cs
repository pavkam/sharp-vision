...
[PublicAPI]
public class Popup: ChromeAuthoringFloatingSurface
{
    ...
    {
        return !IsOpen || Disposed || !HitTestVisible || !EffectiveIsVisible || !EffectiveIsEnabled
            ? null
    ...
    protected override Rect VisualBounds
    {
        get
        {
            ...
            return SurfaceBounds.ExpandVisualBounds(shadow.Visible, shadow.Mode, shadow.Offset);
        }
    }
    ...
    private void OnPopupPropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs eventArgs)
    {
        ...
        if (eventArgs.PropertyName is nameof(Visibility) or nameof(Enabled))
        {
            ReconcilePresentationAvailability();
        }
    ...
    private ControlBase? FindUnavailableAncestor()
    {
        ...
        {
            if (current.Visibility != Visibility.Visible || !current.Enabled)
            {
                return current;
            }
        }
    ...
    private void TrackModalCallbacks(ModalScope scope)
    {
        ...
        Debug.Assert(scope.Active, "A Popup observes only an active modal lifetime.");
    ...
    private void OnModalDismissRequested(object? sender, EventArgs eventArgs)
    {
        ...
        if (sender is ModalScope { Active: true } && IsOpen)
        {
            IsOpen = false;
        }
    }
