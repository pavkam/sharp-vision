...
internal sealed class PopupModalTracker
{
    ...
    /// <param name="popup">The popup whose Opened state is tracked.</param>
    /// <param name="close">Called when the modal scope requests dismissal (typically sets Opened = false).</param>
    ...
    {
        if (_scope?.Active == true || owner.ModalityOwner is not { } modality)
        {
            return;
        }
