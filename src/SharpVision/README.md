...
> [!IMPORTANT]
>
> The matching terminal package is not currently published, so the UI package's
> exact dependency cannot resolve, so it is not currently installable. Use
> repository project references until the terminal package is available.
