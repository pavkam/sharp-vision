...
public readonly record struct ScrollRange
{
    ...
    public ScrollRange(int minimum, int maximum, int value, int viewport)
    {
        ...
        ArgumentException.ThrowIfBelowMinimum(maximum, minimum, nameof(maximum), "Maximum cannot be below minimum.");

        ArgumentOutOfRangeException.ThrowIfOutsideInclusiveRange(value, minimum, maximum, nameof(value), "The value must be inside the inclusive range.");
