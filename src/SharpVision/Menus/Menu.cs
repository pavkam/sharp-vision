...
public sealed class Menu: ItemsControl
{
    ...
    protected override void OnEvent(RoutedEventArgs eventArgs)
    {
        ...
        // unhandled. Consume only an initial Right that can open a child or an initial Left that
        // can close this nested branch. Every other horizontal key keeps bubbling so an owning
        // menu bar can switch top-level siblings.
        if (key.IsInitialKeyDown &&
    ...
    private bool CloseParentSubmenu()
    {
        if (FindAncestor<MenuItem>() is not { IsSubmenuOpen: true } parent ||
            ...
            FindAncestor<Menu>() is not { Orientation: Orientation.Vertical } parentMenu ||
            parentMenu.IndexOfItemControl(parent) < 0)
        {
            return false;
        }
