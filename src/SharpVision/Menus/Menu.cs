...
public sealed class Menu: ItemsControl
{
    ...
    protected override void OnItemControlsChanged()
    {
        ...
        // its owning slot before Disposed itself flips true — IsDisposing is what's already
        // set at this point.
        if (_spacePressedItem is { IsDisposing: true } or { Disposed: true })
        {
    ...
    private bool IsSessionArmed =>
        _modalScope?.Active == true || _isEnteringModalScope || _submenuTransitionDepth > 0;
        ...
        ModalScope? session)
    {
        return ReferenceEquals(menu.FindSessionOwner(), this) &&
            !menu.Disposed && menu.EffectiveIsEnabled && menu.EffectiveIsVisible &&
            menu.IndexOfItemControl(item) >= 0 &&
            !item.Disposed && item.EffectiveIsEnabled && item.EffectiveIsVisible &&
            ...
            (modalityOwner is null ||
                (ReferenceEquals(modalityOwner.Active, session) && session?.Active != false));
    }

    private bool EnsureModalSession()
    {
        if (_modalScope?.Active == true)
        {
            return true;
        }
    ...
    {
        var traversal = new Stack<(Menu Menu, bool Expanded)>();
