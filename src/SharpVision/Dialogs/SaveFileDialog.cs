...
public sealed class SaveFileDialog: FileDialogBase<SaveFileResult>
{
            ...
            SaveFileResult.Cancelled)
    {
        ...
        _saveButton = new Button
        {
            Text = SaveText,
            IsDefault = true,
            Enabled = !string.IsNullOrEmpty(options.InitialFileName)
        };
        Initialize();
    ...
    private void PopulateFileNameFromSelection()
    {
        var selected = FileList.SelectedItems
            ...
            .FirstOrDefault(static entry => !entry.Directory);
    ...
    {
        _saveButton.Enabled = !string.IsNullOrWhiteSpace(_fileNameInput.Text);
