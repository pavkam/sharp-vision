    ...
    where TResult : class
{
    ...
    private static Text CreateEntryContent(object? value)
    {
        ...
        var prefix = entry.Directory ? "▸ " : "· ";
        var suffix = entry.Directory ? Path.DirectorySeparatorChar.ToString() : string.Empty;
    ...
    /// fire-and-forget loop directly and prove the trailing <see cref="ObjectDisposedException"/>
    /// catch guards both the success and failure completion posts - including when the owning
    /// dispatcher is disposed before the load completes - instead of relying on the load's normal
    /// discard, which would turn an unguarded fault into an invisible unobserved task exception.
    /// </summary>
    ...
    {
        if (generation != _loadGeneration || Disposed || Dispatcher is null)
        {
            return;
        }
        ...
        _upButton.Enabled = FileSystem.GetParent(directory) is not null;
    ...
    {
        Loading = value;
        NotifyPropertyChanged(nameof(Loading), InvalidationImpact.None);
    }
    ...
    {
        var folders = entries.Count(static entry => entry.Directory);
