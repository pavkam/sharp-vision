...
public sealed class ConsoleApplicationBuilder
{
    ...
    private readonly Action<string> _writeErrorLine;
    ...
    /// <param name="writeErrorLine">The non-null diagnostic writer for a post-startup fault.</param>
    /// <param name="readEnvironment">
    /// The environment-variable reader backing
    /// <see cref="UseEnvironmentSizeOverrides"/>, or null for the process environment. A seam
    /// because the alternative - a test mutating the real process environment - is shared mutable
    /// state across a parallel test run.
    /// </param>
        ...
        Func<string, string?>? readEnvironment = null)
    {
        ...
        _writeErrorLine = writeErrorLine;
    ...
    /// <remarks>
    /// Both variables must name a positive integer or nothing is overridden. Only the first
    /// observed size is replaced; a genuine resize afterwards always wins.
    /// </remarks>
    ...
    public Application Build()
    {
        ...
        try
        {
            ...
            application = new Application(
                ...
                Options.UseEnvironmentSizeOverrides
            ...
            // UseTheme(...) - otherwise leave whatever OnAttach published alone.
    ...
    internal bool Interactive => _isInteractive();
    ...
    /// <summary>Writes one non-null host diagnostic describing a post-startup fault.</summary>
    /// <remarks>
    /// Separate from <see cref="WriteLine"/> because it targets standard error rather than standard
    /// output, and separate from calling <see cref="ConsoleTextChannel"/> directly so a test can
    /// capture the text and assert on it. Without a seam the only way to exercise the fault report
    /// is to let it reach the real descriptor, where it lands in the shared test host's stderr and
    /// reads as a crash in CI output.
    /// </remarks>
    /// <param name="message">The non-null diagnostic.</param>
    /// <exception cref="ArgumentNullException"><paramref name="message"/> is null.</exception>
    internal void WriteErrorLine(string message)
    {
        ArgumentNullException.ThrowIfNull(message);
        _writeErrorLine(message);
    }
