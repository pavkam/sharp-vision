...
internal static class ColorMath
{
    ...
    internal static Color FromHsv(int hue, double saturation, double value)
    {
        ...
        return Color.Rgb(Component(red + match), Component(green + match), Component(blue + match));
    }
    ...
    internal static void ToHsv(
        ...
        out double value)
    {
        Debug.Assert(color.Kind == ColorKind.Rgb, "Palette colors are resolved before HSV conversion.");
    ...
    /// <param name="color">The RGB background.</param>
    /// <returns>Black for light colors; otherwise white.</returns>
    internal static Color Contrast(Color color)
    {
        Debug.Assert(color.Kind == ColorKind.Rgb, "Contrast receives resolved RGB.");
