...
public sealed class TreeView: CompositeControlBase
{
    ...
    public void SelectAll()
    {
        ...
        _ = CommitSelection(CollectAllItems().Where(static item => item.Enabled), cancellable: true);
    }
    ...
    private void OnKeyRouted(object? sender, KeyEventArgs eventArgs)
    {
        ...
        {
            if (_navigator.Current is TreeViewItem { Checkable: true } checkable)
            {
    ...
    private bool HandleLeft()
    {
        ...
        if (item is { HasChildren: true, Expanded: true })
        {
            item.Expanded = false;
            return true;
        }
    ...
    private bool HandleRight()
    {
        ...
        if (item is { HasChildren: true, Expanded: false })
        {
            item.Expanded = true;
            return true;
        }
    ...
    {
        if (SelectionMode == TreeSelectionMode.None || !item.Enabled)
        {
            return false;
        }
    ...
    private void RebuildFlatList()
    {
        ...
        // ancestor wipe every realized item's selection on the next rebuild.
        var ownedItems = CollectAllItems();
