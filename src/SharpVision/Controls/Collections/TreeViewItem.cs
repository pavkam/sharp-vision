...
#pragma warning restore IDE0032
    ...
    /// <summary>Raised after the <see cref="Expanded"/> state changes.</summary>
    ...
    public bool Expanded
    {
        ...
        set
        {
            ...
            field = value;
            NotifyPropertyChanged(nameof(Expanded), InvalidationImpact.None);
    ...
    {
        if (CheckMark is null && Checkable)
        {
    ...
    /// <inheritdoc/>
    protected override bool CheckedState => IsChecked == true;

    /// <inheritdoc/>
    protected override bool IndeterminateState => IsChecked is null && Checkable;
    ...
    protected override Size MeasureOverride(Constraint constraint)
    {
        ...
        var checkWidth = Checkable ? _checkGapCells + ActualCheckMark.Width : 0;
        return new Size(
            (int) Math.Min(
                int.MaxValue,
                ((long) Depth * indent) +
    ...
    protected override void OnEvent(RoutedEventArgs eventArgs)
    {
            ...
            } && (buttons & Buttons.Primary) != 0 && Bounds.Contains(cells))
        {
            ...
            if (Checkable && cells.X >= checkX && cells.X < checkX + ActualCheckMark.Width)
            {
    ...
    private static bool? EvaluateEffective(TreeViewItem item, Dictionary<TreeViewItem, bool?> memo)
    {
        ...
        if (!item.Checkable || !item.HasChildren)
        {
    ...
    internal bool? GetEffectiveCheckState()
    {
        ...
        while (walk.Count > depth)
        {
            ...
            if (frame.TryTakeNextCheckableChild(out var child))
            {
                ...
                if (child.Checkable && child.HasChildren)
                {
                    walk.Add(new CheckStateFrame(child));
                }
                else
