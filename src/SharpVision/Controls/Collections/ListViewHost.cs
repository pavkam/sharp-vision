...
/// so extent, scrollbar, and offset queries never require a realized row to answer (see #231).
...
internal sealed class ListViewHost: Container
{
    ...
    /// row outside this container's own measure or arrange (see #231).</summary>
    ...
    protected override Size MeasureOverride(Constraint constraint)
    {
        if (RowHeight is int height)
        {
            ...
            // the whole point of #231. Width has no such formula: it is content-derived, so this
