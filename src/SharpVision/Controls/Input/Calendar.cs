...
#pragma warning restore IDE0072
    ...
    private void DrawHeader(TerminalCanvas canvas, Rect bounds)
    {
        ...
        row.DrawRune(ActualStyle.PreviousMonthGlyph, new Point(bounds.X, bounds.Y), style);

        if (bounds.Width > _headerHeight)
        {
            row.DrawRune(ActualStyle.NextMonthGlyph, new Point(bounds.Right - 1, bounds.Y), style);
        }
