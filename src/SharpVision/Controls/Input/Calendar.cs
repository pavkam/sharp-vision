...
public sealed class Calendar: ControlBase, IStyled<CalendarStyle>
{
    ...
    public Calendar()
    {
        _style = InitializeStyle(CalendarStyle.Definition);
    ...
    public CalendarStyle ActualStyle => _style.Actual;
    ...
    public DayOfWeek FirstDayOfWeek
    {
        ...
        set
        {
            ArgumentOutOfRangeException.ThrowIfNotDefined(value, nameof(value), "The weekday is unknown.");
    ...
    internal bool ActivateDate(DateOnly date)
    {
        ...
        // Validate before moving the active date or repaging DisplayMonth: a
...
#pragma warning disable IDE0072 // Unknown or unsupported keys intentionally remain unhandled.
        return stroke.Code switch
        {
            ...
            Code.Enter or Code.Character when IsActivationStroke(eventArgs) => ActivateDate(ActiveDate),
            _ => false
        };
#pragma warning restore IDE0072
    ...
    private void HandlePointer(PointerEventArgs eventArgs)
    {
        ...
        if (pointer.Action == PointerAction.Wheel)
        {
            if (pointer.WheelY != 0)
            {
                eventArgs.IsHandled = MoveDisplayMonth(pointer.WheelY > 0 ? -1 : 1);
            }
        ...
        {
            eventArgs.IsHandled = MoveDisplayMonth(-1);
            return;
        }
    ...
    private DateOnly? DateAt(Point point)
    {
        ...
        if (column is < 0 or >= _columnCount)
        {
            return null;
        }
    ...
    private void DrawHeader(TerminalCanvas canvas, Rect bounds)
    {
        ...
        // A themed glyph is only validated against Ambiguous.Narrow at construction time
        // (Rune.ValidateSingleCell has no policy parameter), so it can still measure to two cells
    ...
    private bool CommitSelection(DateInterval? value)
    {
        ...
        // that applies to repair callers of SetActiveDate. Checking here, before SetActiveDate
        // runs, is what makes that possible; discovering the no-op only after mutating state
        // is too late.
        if (previous == value)
        {
            CaptureFailure(() => SetIntervalAnchor(null), ref failure);
    ...
    private void RepairActiveDate()
    {
        ...
        if (!BlockedDates.Contains(clamped))
        {
            ...
            // SetActiveDate always repages, but repair only needs to move the
