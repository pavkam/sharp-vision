...
public sealed class TextInputContextMenu: ContextMenu
{
    ...
    private void OnOpening(object? sender, EventArgs e)
    {
        ...
        var isReadOnly = _textInput.IsReadOnly;

        _cutItem.IsEnabled = hasSelection && !isPassword && !isReadOnly;
        _copyItem.IsEnabled = hasSelection && !isPassword;
        _pasteItem.IsEnabled = !isReadOnly && ClipboardReader is not null && ClipboardReader().Length > 0;
        _selectAllItem.IsEnabled = _textInput.Text.Length > 0;
        _undoItem.IsEnabled = _textInput.CanUndo;
        _redoItem.IsEnabled = _textInput.CanRedo;
    }
