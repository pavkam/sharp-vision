...
public abstract partial class ControlBase: INotifyPropertyChanged, IDisposable
{
        ...
        where TStyle : struct, IEquatable<TStyle>
    {
        ...
        if (slot.LocalValue.Equals(value))
        {
            return;
        }
        ...
        if (slot.OwnsAppearance)
        {
            ...
            var appearanceState = GetAppearanceState();
            var previousProfile = definition.Appearance!(previousStyle);
            var currentProfile = definition.Appearance(currentStyle);
        ...
        where TStyle : struct, IEquatable<TStyle>
    {
        ...
        return slot.OwnsAppearance
            ? MaximumImpact(
                ...
                this.GetImpact(
                    previous,
                    definition.Appearance!(previousStyle),
                    current,
                    definition.Appearance(currentStyle),
    ...
    private List<(ControlBase Control, bool EffectiveIsVisible, bool EffectiveIsEnabled, bool CanFocus, bool IsTabStop)>
    ...
    private static void PublishDerivedFocusStateChanges(
        List<(ControlBase Control, bool EffectiveIsVisible, bool EffectiveIsEnabled, bool CanFocus, bool IsTabStop)> before)
    {
        ExceptionDispatchInfo? failure = null;
    ...
    protected virtual ThemeRole ThemeRole => _primaryStyle?.Role ?? ThemeRole.Control;
    ...
    protected virtual ThemeProfile AppearanceProfile =>
        _primaryStyle?.GetAppearance(Theme) ?? (Theme ?? Themes.Dark).GetProfile(ThemeRole);
