...
public abstract partial class ControlBase: INotifyPropertyChanged, IDisposable
{
    ...
    public bool EffectiveIsEnabled => Enabled && (Parent?.EffectiveIsEnabled ?? true);
    ...
    public bool CanFocus => Focusable && EffectiveIsVisible && EffectiveIsEnabled && !Disposed;
    ...
    public virtual bool CanTabStop => CanFocus && TabStop;
    ...
    public bool Focused => _interactionState.Focused;
    ...
    public bool PointerOver => _interactionState.PointerOver;

    /// <summary>Gets whether the pointer directly targets this control.</summary>
    public bool PointerDirectlyOver => _interactionState.PointerDirectlyOver;
    ...
    public bool Pressed => _interactionState.Pressed;
    ...
    /// <summary>Raised after the <see cref="Enabled"/> property changes.</summary>
    ...
    //   with NotifyPropertyChanged(nameof(Opened), InvalidationImpact.Measure) (Popup.cs). Control's
    ...
    {
        if (!_interactionState.SetFocused(value) || Disposed || Dispatcher is null)
        {
            return;
        }
    ...
    internal void SetPointerOver(bool value, bool directlyOver)
    {
        ...
        InvalidateVisualStateCore();
        PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(nameof(PointerOver)));
        PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(nameof(PointerDirectlyOver)));
    ...
    {
        Debug.Assert(!Disposed, "A disposed control cannot measure content.");
        return default;
    }
    ...
    protected virtual void ArrangeOverride(Rect bounds) =>
        Debug.Assert(!Disposed, "A disposed control cannot arrange content.");
    ...
    protected virtual void OnFocusChanged(bool focused) =>
        Debug.Assert(!Disposed, "A disposed control cannot change focus state.");
    ...
    protected virtual void OnPressedChanged(bool pressed) =>
        Debug.Assert(!Disposed, "A disposed control cannot change pressed state.");
    ...
    protected virtual void OnDisposing() =>
        Debug.Assert(!Disposed, "The disposing hook runs before disposal commits.");

    /// <summary>Releases derived completion state after disposal and ownership removal have committed.</summary>
    /// <remarks>
    /// The hook runs at most once with <see cref="Disposed"/> true, a null parent, and no dispatcher.
    /// Failures join the ordered disposal aggregation without undoing committed cleanup.
    /// </remarks>
    protected virtual void OnDisposed()
    {
        Debug.Assert(Disposed, "The disposed hook runs after disposal commits.");
    ...
    protected virtual void OnParentChanged(ControlBase? previous, ControlBase? current)
    {
        _ = previous;
        _ = current;
        Debug.Assert(!Disposed, "A disposed control cannot change parent.");
    ...
    {
        _ = canvas.Bounds;
        Debug.Assert(!Disposed, "A disposed control cannot render content.");
    }
    ...
    {
        _ = canvas.Bounds;
        Debug.Assert(!Disposed, "A disposed control cannot render an adornment.");
    }
    ...
    {
        Debug.Assert(!Disposed, "A disposed control cannot render children.");
    ...
    {
        _ = canvas.Bounds;
        Debug.Assert(!Disposed, "A disposed control cannot render overlay chrome.");
    }
    ...
    /// </remarks>
    protected virtual bool CheckedState => false;
    ...
    /// </remarks>
    protected virtual bool PressedState => _interactionState.Pressed;
    ...
    /// </remarks>
    protected virtual bool SelectedState => _interactionState.Selected;

    /// <summary>Gets whether this control is the current member of an owning navigator.</summary>
    protected virtual bool CurrentState => _interactionState.Current;
    ...
    /// </remarks>
    protected virtual bool IndeterminateState => false;
    ...
    /// Enabled) has committed. Nothing has changed yet when this runs, so each live property
    ...
    {
        get => LocalBorderValue ?? GetNormalAppearance().Border;
        set => SetBorder(value);
    }
    ...
    /// <summary>Clears the complete local border and returns ownership to the active semantic appearance.</summary>
    ...
    {
        get => LocalShadowValue ?? GetNormalAppearance().Shadow;
        set => SetShadow(value);
    }
    ...
    /// <summary>Clears the complete local shadow and returns ownership to the active semantic appearance.</summary>
    ...
    protected void SetAppearance(VisualState state, AppearanceOverlay? appearance)
    {
        ...
        // For a control whose ambient face is its ACTIVE state rather than its Normal - PressableBase
        // and ListItem, which override StateAffectsAmbientAppearance - a per-state overlay is folded
        // into the face descendants inherit, so clearing this control alone leaves them holding a
        // ResolvedAppearance built from the previous overlay. They cannot take the render-clean-reuse
        // fast path either (that needs an opaque background), so they repaint from the stale cache
        // and a Button's caption keeps its old colour while the button face changes. This mirrors
        // what InvalidateVisualStateCore already does for the same two controls.
