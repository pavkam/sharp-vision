...
public abstract class HeaderedContentControl: ContentControl
{
    ...
    /// <summary>Measures the concrete owner's own header and content within the supplied
    /// content constraint.</summary>
    ...
    /// <remarks>
    /// <see cref="ContentControl"/>'s inherited implementation only measures <see cref="ContentControl.Content"/>,
    /// so a concrete owner must declare this explicitly or <see cref="Header"/> is silently never
    /// measured or arranged. A specialized owner retains full control over how the header and content
    /// are combined alongside its own chrome.
    /// </remarks>
    protected abstract override Size MeasureOverride(Constraint constraint);

    /// <summary>Arranges the concrete owner's own header and content within the committed content box.</summary>
