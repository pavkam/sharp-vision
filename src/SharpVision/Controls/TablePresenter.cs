...
internal sealed class TablePresenter: Container
{
    ...
    protected override void ArrangeOverride(Rect bounds)
    {
        ...
        for (var rowIndex = 0; rowIndex < _owner.Rows.Count; rowIndex++)
        {
            ...
            for (var columnIndex = 0; columnIndex < _owner.Columns.Count; columnIndex++)
            {
                ...
                ArrangeChild(row.Cells[columnIndex], _owner.CellPadding.Deflate(slot), ResolvedAxes.Both);
    ...
    {
        if (_owner.Columns.Count == 0 || Bounds.Width == 0 || Bounds.Height == 0)
        {
            return;
        }
        ...
        if (_owner.ShowHeader)
        {
            ...
            {
                var area = new Rect(x, Bounds.Y + _owner.CellPadding.Top, ColumnWidths[index], 1);
        ...
        var y = Add(Bounds.Y, headerHeight + (_owner.ShowHeader ? RowGap : 0));
