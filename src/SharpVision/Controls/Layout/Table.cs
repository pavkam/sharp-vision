...
public sealed class Table: ItemsControl
{
    ...
    public bool Editing => _edit is not null;
    ...
    public bool BeginEdit(TableRow row, int columnIndex)
    {
        ...
        if (Columns[columnIndex].ReadOnly || row.Cells[columnIndex] is not TextInput editor || editor.ReadOnly)
        {
            return false;
        }
        ...
        NotifyPropertyChanged(nameof(Editing), InvalidationImpact.None);
        return true;
    }
    ...
    private void ValidateRow(TableRow row)
    {
        ...
        foreach (var cell in row.Cells)
        {
            if (cell.Parent is not null || cell.Disposed)
            {
