...
public sealed class FocusManager: IDisposable
{
    ...
    {
        Debug.Assert(
            Disposed || ReferenceEquals(Root.FocusOwner, this),
    ...
    private void CapturePendingPriorFailure(ref ExceptionDispatchInfo? failure)
    {
        ...
        {
            if (request.PriorFailure is not null)
            {
                failure = request.PriorFailure;
                break;
            }
        }
    }
        ...
        while (pending.Count > 0)
        {
            ...
            CaptureFailure(
                () => request.Completion?.Invoke(
                    false,
                    request.PriorFailure?.SourceException),
                ref failure);
        }
    }
        ...
        bool pumpPending = true)
    {
        ...
        try
        {
            try
            {
                ...
                else
                {
                    ...
                    if (!Disposed && previous is not null)
                    {
                        previous.PublishLostFocus();

                        for (var index = previousPath.Count - 1;
                            index >= commonLength && !Disposed;
                            index--)
                        {
                    ...
                    if (!Disposed && control is not null)
                    {
                        for (var index = commonLength;
                            index < currentPath.Count && !Disposed;
                            index++)
                        {
        ...
        finally
        {
            ...
            if (completion is not null)
            {
                ...
                CaptureFailure(
                    () => completion(
                        !Disposed && outcomeKnown && committed && ReferenceEquals(Focused, control),
                        outcomeFailure),
                    ref requestFailure);
            }
    ...
    private void PumpPendingRequests()
    {
        ...
        try
        {
            while (PendingRequests is { Count: > 0 } pending)
            {
                ...
                aggregateFailure ??= requested.PriorFailure;
                CapturePendingPriorFailure(ref aggregateFailure);
                var requestFailure = requested.PriorFailure;

                try
                {
                    if (requested.Scope is { Active: false } ||
                        requested.Valid?.Invoke() is false)
                    {
                        requested.Completion?.Invoke(
                            false,
                            requested.PriorFailure?.SourceException);
                    }
                    else if ((requested.Control is null || IsMember(requested.Control)) &&
                        IsAllowed(requested.Control) &&
                        IsEligibleTarget(requested.Control))
                    {
                        _ = Change(
                            requested.Control,
                            requested.Reason,
                            requested.Cancellable,
                            publishChanging: true,
                            requested.Completion,
                            requested.PriorFailure);
                    }
                    else
                    {
                        requested.Completion?.Invoke(
    ...
    private bool IsEligible(ControlBase control) =>
        ...
        {
            Disposed: false, Dispatcher: not null, CanFocus: true, EffectiveIsVisible: true, EffectiveIsEnabled: true
        };
