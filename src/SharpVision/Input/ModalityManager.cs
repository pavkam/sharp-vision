...
public sealed class ModalityManager: IDisposable
{
    ...
    internal void Include(ModalScope scope, ControlBase root)
    {
        ...
        if (!scope.Active || !_stack.Contains(scope))
        {
    ...
    internal void Unavailable(ControlBase subtree, bool restoreFocus)
    {
        ...
        {
            if (_stack[index].Active && IsWithin(_stack[index].Root, subtree))
            {
                primaryIndex = index;
                break;
            }
        ...
        ExceptionDispatchInfo? priorFailure = null)
    {
        ...
        Debug.Assert(_stack[index].Active, "Modal unwind begins at one interaction-active scope.");
    ...
    private void PumpUnwinds()
    {
        ...
        try
        {
            while (_pendingExit is null)
            {
                ...
                var scope = _stack[^1];
                Debug.Assert(!scope.Active, "The youngest unwind lifetime committed inactive before publication.");
    ...
    private void CommitInactiveFrom(int index)
    {
        ...
        try
        {
            ...
            {
                if (_stack[current].Active)
                {
                    _stack[current].Active = false;
                    changed = true;
                }
            }
    ...
    private void CompletePendingExit(ModalScope scope, Exception? focusFailure)
    {
        ...
        if (_unwindRestoresFocus && !_focus.Disposed)
        {
            CaptureFailure(
    ...
    private bool IsFocusCoherent(ControlBase? target, ControlBase? displaced) =>
        ...
        (target is null || target.Focused) &&
        (displaced is null || ReferenceEquals(displaced, target) || !displaced.Focused);
        ...
        ControlBase? entryPreviousFocus)
    {
        ...
        _focus.Focus(
            ...
            (committed, failure) =>
            {
                if (entryScope is not null && !entryScope.Active)
                {
                    return;
                }
    ...
    private bool IsEligibleFocusTarget(
        ...
        IsMember(control) && !control.Disposed && control.Dispatcher is not null &&
