...
internal static class FigletParser
{
    ...
    /// <exception cref="FormatException">The payload violates FIGfont syntax.</exception>
    /// <exception cref="InvalidDataException">A structural limit is exceeded.</exception>
    internal static FigletFont Parse(ReadOnlySpan<byte> bytes, string name, FigletLimits limits)
    {
        string text;
        ...
        {
            throw new InvalidDataException("The FIGfont height exceeds the configured limit.");
        }
        ...
        {
            throw new InvalidDataException("The FIGfont row width exceeds the configured limit.");
        }
        ...
        while (cursor < lines.Length && glyphs.Count < limits.MaxGlyphs)
        {
            ...
            {
                throw new FormatException($"The FIGfont code tag '{token}' is not a Unicode scalar.");
            }
        ...
        return cursor < lines.Length && lines[cursor..].Any(line => line.Length != 0)
            ? throw new InvalidDataException("The FIGfont glyph count exceeds the configured limit.")
            : new FigletFont(
                name,
                height,
                baseline,
        ...
        for (var row = 0; row < height; row++)
        {
            ...
            if (encoded.Length == 0 || encoded.Length > limits.MaxRowWidth)
            {
                throw new FormatException(
                    $"FIGfont glyph row {cursor} has length {encoded.Length}; expected 1 through " +
                    $"{limits.MaxRowWidth}.");
            }
    ...
    private static string ReadLine(string[] lines, ref int cursor, string area) =>
        cursor < lines.Length
            ? lines[cursor++]
            : throw new FormatException($"The FIGfont ended inside {area}.");
    ...
    private static int CodePoint(string value)
    {
        ...
        if (radix == 8)
        {
            ...
            {
                throw new FormatException($"The FIGfont code tag '{value}' is invalid.", exception);
            }
        }
    ...
    private static FigletDirection Direction(int value) => value switch
    {
        ...
        _ => throw new FormatException("The FIGfont direction is unknown."),
    };
