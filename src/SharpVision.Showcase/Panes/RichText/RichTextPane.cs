...
namespace SharpVision.Showcase.Panes.RichText;

/// <summary>Documents and demonstrates the RichText control.</summary>
internal sealed class RichTextPane: ShowcasePane
{
    ...
    [
        PaneMetadata.Interaction("Inline mutation", "Add or edit a Run, Hyperlink, or LineBreak", "The document invalidates and remeasures its formatted content."),
        PaneMetadata.Interaction("Pointer", "Activate a Hyperlink", "The hyperlink event receives the clicked semantic target."),
        PaneMetadata.Interaction("Resize", "Change the available width", "Wrapping and line alignment recompute without splitting grapheme clusters."),
    ];

    private static readonly PropertyDescription[] _catalogProperties =
    [
        PaneMetadata.Property("Inlines", "Inlines", "empty", "Owns the ordered Run, Hyperlink, and LineBreak values that form the displayed document."),
        PaneMetadata.Property("Wrapping", "Wrapping", "Word", "Defaults to word-aware wrapping; applications may preserve logical lines or choose grapheme wrapping explicitly."),
        PaneMetadata.Property("TextAlignment", "Alignment", "Start", "Places every formatted document line at the start, center, or end of its content box."),
        PaneMetadata.Property("Padding", "Thickness", "0", "Adds internal terminal-cell space around the formatted inline document."),
    ];

    /// <summary>Initializes the RichText showcase page and composes its specimens.</summary>
    internal RichTextPane()
        : base("RichText", _catalogSummary, _catalogInteractions, _catalogProperties)
    {
    }
    ...
    protected override void BuildExamples(ControlStack examples)
    {
        ...
        attributes.Inlines.Add(new ControlRun("diagnostic emphasis")
        {
            ...
            Underline = global::SharpVision.Terminal.Protocols.Underline.Curly,
