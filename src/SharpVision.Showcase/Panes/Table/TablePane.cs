...
namespace SharpVision.Showcase.Panes.Table;

/// <summary>Documents and demonstrates the Table control.</summary>
internal sealed class TablePane: ShowcasePane
{
    ...
    [
        PaneMetadata.Interaction("Columns", "Choose fixed, automatic, percentage, or fill widths", "Each header and row cell shares one resolved track."),
        PaneMetadata.Interaction("Rows", "Add detached controls matching the column count", "Cells are owned by generated borders while their semantic controls remain interactive."),
        PaneMetadata.Interaction("Resize", "Change the available table width", "Percentage and fill columns recompute through the shared Grid allocator."),
        PaneMetadata.Interaction("Pointer and keyboard", "Interact with a focusable cell control", "The cell control receives normal routed input without bypassing the table layout."),
    ];

    private static readonly PropertyDescription[] _catalogProperties =
    [
        PaneMetadata.Property("Columns", "TableColumns", "empty", "Owns non-empty titled fixed, automatic, percentage, or proportional column definitions."),
        PaneMetadata.Property("Rows", "TableRows", "empty", "Owns rows whose detached controls exactly match the defined column count."),
        PaneMetadata.Property("ShowHeader", "bool", "true", "Renders a padded header row from each TableColumn Header value."),
        PaneMetadata.Property("CellPadding", "Thickness", "0", "Deflates every header and data cell with non-negative terminal-cell padding."),
        PaneMetadata.Property("RowSpacing / ColumnSpacing", "int", "0 / 0", "Add non-negative space between cells while preserving contained track geometry."),
        PaneMetadata.Property("ShowGridLines", "bool", "true", "Draws light Unicode lines in available table gaps using the configurable grid-line foreground."),
    ];

    /// <summary>Initializes the Table showcase page and composes its specimens.</summary>
    internal TablePane()
        : base("Table", _catalogSummary, _catalogInteractions, _catalogProperties)
    {
    }
