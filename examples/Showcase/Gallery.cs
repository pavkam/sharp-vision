...
public sealed class Gallery: Screen
{
    ...
    [
        ...
        ("Input", NumberInputPane.Title, static () => new NumberInputPane()),
        ("Display", CanvasPane.Title, static () => new CanvasPane()),
        ("Charts", HorizontalBarChartPane.Title, static () => new HorizontalBarChartPane()),
        ("Charts", VerticalBarChartPane.Title, static () => new VerticalBarChartPane()),
        ("Charts", LineChartPane.Title, static () => new LineChartPane()),
        ("Charts", AreaChartPane.Title, static () => new AreaChartPane()),
        ("Charts", SparklinePane.Title, static () => new SparklinePane()),
        ...
        ("Input", CommandBarPane.Title, static () => new CommandBarPane()),
        ("Input", CommandBarItemPane.Title, static () => new CommandBarItemPane()),
        ("Input", CommandBarSeparatorPane.Title, static () => new CommandBarSeparatorPane()),
        ...
        ("Navigation", BreadcrumbItemPane.Title, static () => new BreadcrumbItemPane()),
    ...
    // SharpVision.Styling.ThemeCatalog, dark group first then light, preserving each
    ...
    private readonly FigletText _productIdentity;
    private readonly ComboBox _themePicker;
    private readonly Button _quit;
    private readonly string[] _fullProductIdentityLines;
    ...
    public Gallery()
    {
        ...
        (ApplicationBar, _productIdentity, _fullProductIdentityLines) =
            CreateApplicationBar(_themePicker, _quit);
    ...
    {
        ArgumentNullException.ThrowIfNull(application);
        UpdateProductIdentity(application.Size.Width);
    ...
    private static (Dock ApplicationBar, FigletText ProductIdentity, string[] FullProductIdentityLines)
        ...
        Button quit)
    {
        ...
        var font = FigletCatalog.Default.Load("Classy");
        var title = new FigletText(font)
        {
            Content = "SharpVision",
            Face = new Face(
        ...
        var applicationBar = new Dock
        {
            Height = Length.Cells(9),
            Padding = new Thickness(horizontal: 1, vertical: 0),
            Children = { actions, title }
        };
        var fullProductIdentityLines = font.Render("SharpVision").Split('\n');

        return (applicationBar, title, fullProductIdentityLines);
    }
    ...
    private void OnApplicationResize(object? sender, ResizeEventArgs eventArgs)
    {
        ...
        UpdateProductIdentity(eventArgs.Dimensions.Cells.Width);
    }

    private void UpdateProductIdentity(int viewportWidth)
    {
        var fullProductIdentityWidth = _fullProductIdentityLines.Max(line => MeasureCells(line));
        var requiredWidth = fullProductIdentityWidth +
            _themePicker.DesiredSize.Width +
            _quit.DesiredSize.Width +
            ApplicationBar.Padding.Horizontal +
            1;
        var content = requiredWidth > viewportWidth
            ? "SV"
            : "SharpVision";

        if (_productIdentity.Content != content)
        {
            _productIdentity.Content = content;
        }
    }
    ...
    private void OnThemeSelected(object? sender, ListSelectionChangedEventArgs eventArgs)
    {
        ...
        {
            SetTheme(ThemeCatalog.Load(_themePickerEntries[index].Slug));
        }
    }
