...
internal sealed class TreeViewPane: CompositeControlBase
{
    ...
    private static DocPage CreateContent()
    {
        ...
        // StartAffix reserves a fixed cell for an application-owned unsaved-change marker.
        var unsavedTree = new TreeView
        {
            Width = Length.Cells(34),
            Height = Length.Cells(4)
        };
        var scratch = new TreeViewItem("scratch") { IsExpanded = true };
        scratch.Children.Add(new TreeViewItem("Notes.md") { StartAffix = new Affix("●", "*", SemanticColor.Warning) });
        scratch.Children.Add(new TreeViewItem("README.md"));
        unsavedTree.Items.Add(scratch);
        ...
        return new DocPage(Title,
            ...
            new DocSection("\U0001f4dd", "Affixes",
                "<info>StartAffix</info> and <info>EndAffix</info> reserve a fixed cell beside an item's header, inboard of the disclosure and check glyphs, for application-owned, data-driven decoration that a theme never authors.",
                new DocExample("Unsaved-change marker",
                    "The dot marks a file with pending edits; it sits in its own reserved cell before the header and never shares space with the disclosure glyph.",
                    unsavedTree,
                    "var item = new TreeViewItem(\"Notes.md\")\n{\n    StartAffix = new Affix(\"●\", \"*\", SemanticColor.Warning)\n};")));
    }

    private static DocSection CreateAsyncSection()
    {
        var source = new ShowcaseAsyncChildSource();
        var documents = new TreeViewItem("Documents (async)") { ChildSource = source };
        var flaky = new TreeViewItem("Flaky service (fails once)") { ChildSource = source };
        var asyncTree = new TreeView
        {
            Width = Length.Cells(42),
            Height = Length.Cells(9)
        };
        asyncTree.Items.Add(documents);
        asyncTree.Items.Add(flaky);

        const string recipe = """
            var tree = new TreeView { Width = Length.Cells(42) };
            var documents = new TreeViewItem("Documents") { ChildSource = source };
            tree.Items.Add(documents);

            // ITreeViewChildSource.GetChildrenAsync answers this item's
            // request once, asynchronously; the tree shows a "Loading…"
            // row until it commits, or a retryable failed row on error.
            """;

        return new DocSection("⏳", "Asynchronous children",
            "Assigning <info>ChildSource</info> instead of authoring <info>Children</info> " +
            "directly makes an item load its children the first time it expands. " +
            "<info>Documents (async)</info> shows the \"Loading…\" row while its request is in " +
            "flight, then reveals a file, a note, and an empty folder that itself commits with " +
            "no children. <info>Flaky service</info> fails its first request - click the failed " +
            "row or press Enter while it is current to retry.",
            new DocExample("Documents, an empty folder, and a flaky service",
                "Expand each node to watch its own request run. Retry the failed row by clicking " +
                "it or pressing Enter while it is the current item.",
                asyncTree,
                recipe));
    }

    /// <summary>Answers every request after a short delay, so the showcase page visibly passes
    /// through the Loading status row rather than committing instantly.</summary>
    private sealed class ShowcaseAsyncChildSource: ITreeViewChildSource
    {
        private static readonly TimeSpan _delay = TimeSpan.FromMilliseconds(700);
        private int _flakyAttempts;

        public async Task<IReadOnlyList<TreeViewChildDescription>> GetChildrenAsync(
            TreeViewChildContext context,
            CancellationToken cancellationToken)
        {
            await Task.Delay(_delay, cancellationToken);

            return context.Header switch
            {
                "Documents (async)" =>
                [
                    ...
                    new TreeViewChildDescription("empty", "Empty folder")
                ],
                "Empty folder" => [],
                "Flaky service (fails once)" => AnswerFlaky(),
                _ => []
            };
        }

        private IReadOnlyList<TreeViewChildDescription> AnswerFlaky()
        {
            _flakyAttempts++;

            return _flakyAttempts == 1
                ? throw new InvalidOperationException("Simulated transient failure - retry to recover.")
                : [new TreeViewChildDescription("recovered", "Recovered.log") { Presence = TreeViewChildPresence.Leaf }];
        }
    }
