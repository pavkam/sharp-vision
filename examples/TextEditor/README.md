...
`TextInput.ContextMenu` with Cut, Copy, Paste, Select All, Find, and Replace
actions. The `ContextMenu` wrapper opens the menu at the pointer and owns its
own light dismiss, so outside input closes the menu without replaying that input
into the editor.
