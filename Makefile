...
test-ci: build
	...
	@dotnet test --project tests/SharpVision.Document.Tests --configuration $${CONFIGURATION:-Release} --no-build --minimum-expected-tests 240 --timeout $${TEST_TIMEOUT:-900s} --coverage --coverage-settings $(COVERAGE_SETTINGS) --coverage-output-format cobertura --report-xunit-trx
	@dotnet test --project tests/SharpVision.FigletFonts.Tests --configuration $${CONFIGURATION:-Release} --no-build --minimum-expected-tests 22 --timeout $${TEST_TIMEOUT:-900s} --coverage --coverage-settings $(COVERAGE_SETTINGS) --coverage-output-format cobertura --report-xunit-trx
	@dotnet test --project tests/SharpVision.SyntaxHighlighting.Tests --configuration $${CONFIGURATION:-Release} --no-build --minimum-expected-tests 280 --timeout $${TEST_TIMEOUT:-900s} --coverage --coverage-settings $(COVERAGE_SETTINGS) --coverage-output-format cobertura --report-xunit-trx
	@dotnet test --project tests/SharpVision.Compatibility.Tests --configuration $${CONFIGURATION:-Release} --no-build --minimum-expected-tests 5 --timeout 300s --report-xunit-trx
...
docs-samples:
	...
	@dotnet build src/SharpVision.SyntaxHighlighting/SharpVision.SyntaxHighlighting.csproj --configuration Release --no-restore
...
lint: restore docs-samples
	...
	@npm run check:syntax-definitions
