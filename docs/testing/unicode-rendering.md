...
multiplication markers and compared against `Graphemes.Enumerate`; curated cases
separately name CR/LF, Hangul, Indic, flags, modifiers, keycaps, ZWJ families,
and invalid UTF-16. A warmed mixed-text loop must allocate zero managed bytes.
...
boundaries, U+FFFD ownership in frames, the exact replacement UTF-8 in both full
and incremental output, and no mutation of the neighboring preceding cell. The
independent terminal model classifies test input without calling production
width code.
...
orphan marks, VS15/VS16, modifiers, keycaps, flags, tag flags, ZWJ families and
professions, private and unassigned values, C0/C1 controls, tabs and newlines,
and invalid UTF-16.

## Frame oracle

For frames A and B: render A fully into a virtual terminal, apply the production
incremental diff to reach B, then compare screen cells, grapheme ownership,
cursor, style, hyperlink, and modes against a second virtual terminal that
rendered B fully. This runs for targeted transitions and for seeded random frame
pairs.
...
after each random draw or clear operation and prints the seed plus operation on
failure.
...
allocation-free, so that one-time tiered-runtime bookkeeping is not mistaken for
...
the remaining combining cluster in a fresh semantic frame and in terminal
output. The nested automatic-scroll proof renders wide CJK content through two
clipped viewports while offsets and pixel-derived pointer cells change, then
asserts that no orphan continuation survives at the exposed frame edge.
