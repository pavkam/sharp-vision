...
only for a nearest-connection xterm hint when Kitty is not hinted. The query
stays raw even with an approved outer route, matching the destination of the
keyboard enable; outer identity cannot authorize local keyboard status. A native
Windows connection accepts the built-in `windows-vt` description name as the
same hint, since `TERM` is essentially never set there and conhost answers an
unrecognized DECRQSS status with a safe negative reply. A matched reply produces
