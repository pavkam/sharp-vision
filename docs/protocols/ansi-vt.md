...
`Csi`, `Sgr`, `Osc`, and `ProtocolModes` provide the exact-byte output subset
documented on their own pages. `ProtocolParser` preserves unknown valid ESC/CSI
functions as borrowed events instead of guessing a terminal identity. The
built-in Windows VT description owns the exact compiled subset described below.
Runtime preflight selects its profile, and the session consumes the matched
lifecycle subset.
