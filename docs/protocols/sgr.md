...
The semantic `CellStyle` stores rapid blink and overline attributes plus a typed
`Underline` and independent `UnderlineColor`. Legacy
`TerminalAttributes.Underline` continues to mean standard SGR 4. Slow and rapid
blink are mutually exclusive; legacy and typed underline selections are mutually
