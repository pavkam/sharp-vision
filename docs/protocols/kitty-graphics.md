...
`Clipboard.TransferLimits.MaxMetadataBytes`. `Response.Message` exposes that
bounded but untrusted terminal text to an explicit caller. Diagnostics and
`ToString()` never include it. Malformed, duplicate-field, overflowing,
unknown-field, late, and unrelated replies cannot consume another transaction.
