...
| `Feature.IsSupported`                    | Whether the state reports support.                             |
| `Feature.IsAuthoritative`                | Whether the origin is strong enough to authorize typed output. |

`TerminalProtocol` includes synchronized output, focus, paste, cell and pixel
mouse, Kitty keyboard, OSC 52 and Kitty clipboard, Kitty graphics, sixel, iTerm2
images, styled underlines, underline color, and overline. Some families have
deliberately stricter evidence rules:
...
| `IClipboard`       | Requires authoritative OSC 52 evidence and, for a database, a valid two-argument `Ms` program. | Emits no bytes.    |

A lone or parameterized terminfo `TS` status-line program is not treated as
OSC 2. The typed Kitty OSC 5522 extension is not yet connected to this
application facade; its [protocol page](kitty-clipboard.md#supported-features)
owns that implementation gap. Bell, title, and clipboard bytes use the
