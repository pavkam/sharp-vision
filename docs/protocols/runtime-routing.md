...
[`ProtocolParser`](ecma-48.md#streaming-grammar) owns bounded ECMA-48 framing.
...
`IMetricsResponseSink` for window/cell geometry. `InputDecoder` dispatches each
...
before a diagnostic is delivered; it never replays retained bytes as input. A
buffered multiplexer reply-prefix candidate and its discard-recovery sibling
