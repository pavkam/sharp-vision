...
1003, 1004, 1005, 1006, 1015, 1016, 1049, 2004, 2026, and 5522.
`Csi.QueryPrivateMode` emits DECRQM; `XtermResponses.TryCsi` validates DECRPM
and maps states 1/2/3 to supported while 0/4 remain unsupported, except mode
2026 where state 3 is also treated as unsupported (see above). `QueryTracker`
bounds in-flight queries, rejects ambiguous duplicate uncorrelated requests,
correlates Kitty IDs, and distinguishes duplicate from late replies using an
injected `TimeProvider`. Mode ownership and reverse-order terminal restoration
are implemented by
