...
known lies. The startup batch also issues the Kitty OSC 5522 DECRQM probe, whose
typed correlation family the
[Kitty clipboard page](kitty-clipboard.md#supported-features) describes.
...
`XtermResponses.TryCsi` recognizes a bounded `CSI ? flags u` reply as
...
[color-evidence lattice](../architecture/capabilities.md#overview). An explicit
