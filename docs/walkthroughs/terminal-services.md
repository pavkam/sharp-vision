...
{
    if (application.Terminal.TitleSupported)
    {
        application.Terminal.SetTitle("SharpVision dashboard");
    }

    if (application.Terminal.Bell.Supported)
    {
        application.Terminal.Bell.Ring();
    }

    if (application.Terminal.Clipboard.Supported)
    {
...
`Clipboard.Supported` reports authoritative evidence for either protocol: Kitty
OSC 5522 or OSC 52. `Write` and `Request` prefer Kitty when proven, falling back
to OSC 52 text, per the
...
terminal - success, terminal failure, cancellation, or timeout - raises
`KittyClipboardReplyReceived` once. A `Write` that fell back to OSC 52 raises
nothing: that protocol defines no acknowledgement for a write, so there is no
outcome to report. Check the Kitty clipboard capability before making a "copied"
confirmation depend on the event.
...
override. Prefer a typed service's `Supported` member when one exists for the
