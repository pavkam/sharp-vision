...
| Edit a nullable `decimal` through a transient buffer with numeric-specific formatting/validation seams                         | [`NumericInputBase`](../controls/input/numeric-input-base.md#overview)                   | Value-editor ownership from `InputBase`, plus the numeric buffer/format/validate seams                               |
| Edit a segmented calendar/clock value (`DateOnly`/`TimeOnly`/`DateTime`) through per-segment digit entry                       | [`TemporalInputBase<TValue>`](../controls/input/temporal-input-base.md#overview)         | Value-editor ownership from `InputBase`, plus the segment digit/increment/clear seams                                |
| Retain private implementation controls                                                                                         | `CompositeControlBase`                                                                   | Constructor creates one permanent private root                                                                       |
| Retain a private composite whose implementation includes one bounded, two-axis scrolling host                                  | [`ScrollableCompositeControlBase`](../controls/scrollable-composite-control.md#overview) | Constructor creates the private root and installs one scrolling host                                                 |
