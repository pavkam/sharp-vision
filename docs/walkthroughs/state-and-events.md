...
{
    increment.Enabled = args.Current == true;
};
```

Properties commit before their change events run. `Enabled` participates in the
inherited effective state, so disabling an ancestor also disables this button.
See the [`CheckBox` event order](../controls/input/check-box.md#api) and the
[`ControlBase` inherited-state rules](../controls/control.md#api).
