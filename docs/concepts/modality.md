...
policy, and `Active`. `DismissRequested` reports qualifying outside input,
...
repairs the manager's `Focused` identity and the controls' `Focused` facts
...
`Exited` once, after `Active` becomes false.
...
The complete requested scope batch commits `Active = false` synchronously,
...
for choosing another policy or initial focus, and it may begin only while the
popup is closed. Any ordinary close or external scope exit closes the
presentation and exits its scope before the content becomes unavailable, so
focus can restore through the still-attached subtree. A failed entry recloses a
popup that the call exposed.
...
the plane armed. Leaf invocation, a final Escape, or an outside dismissal closes
the chain and exits the scope. A top menu inside a modal window temporarily
becomes a younger child scope, and closing it restores the parent dialog plane.
