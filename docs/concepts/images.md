...
fails the remaining bounded frame output budget reports that limit separately. A
placement that was itself encodable but was forced to fall back solely because a
later, higher-stacked overlapping placement could not be encoded reports that
overlap block separately. Diagnostic construction rejects the empty image
identity and undefined reasons. The valid default `Metrics` value exposes an
empty diagnostic snapshot, never null.
