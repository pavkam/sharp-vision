...
both controls' `Focused` state before raising `Lost` and `Gained`. If the tree
...
eligible member - for example a `TabStop=false` leaf that was focused by a
...
`Active` remain separate pieces of state.
...
first, then synchronously clears `FocusManager.Focused` and `Focused`. This
