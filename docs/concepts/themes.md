...
Twenty-eight library controls resolve part or all of their structural `Style`
...
| `sharpVision.document`    | `Document`                | `control`      | Optional `SharpVision.Document` package: `headingFace`, `markerFace`, `quoteFace`, `codeFace`, `ruleFace`, `calloutFace`, `calloutTitleFace`, `tableFace`, `tableHeaderFace`, `linkFace`, `activeLinkFace`, `disabledLinkFace`, `actionLinkFace`, `activeActionLinkFace` (faces), `glyphs` (five one-character strings) |
