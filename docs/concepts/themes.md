...
| `table`                   | `Table`                   | `control`      | `glyphs` (five one-character strings), `cellPadding` (`{x, y}` object), `headerForeground`, `headerBackground`, `gridLineColor` (nullable colors - null inherits the table's own face)                                                                     |
