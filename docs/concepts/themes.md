...
`Theme.ResolveColor(SemanticColor)` is the typed public lookup.
...
Past `Normal`, only **`input`** keeps following `control`. For each other state,
`input` inherits `control`'s _delta_ for that state - what `control.pointerOver`
changed relative to `control.normal` - applied onto `input`'s own resolved
`Normal`, with `input`'s own JSON for that state winning on top. Inheriting the
delta rather than the whole value is what keeps `input`'s own `border` sides and
glyph style intact, so a hover never silently re-measures a field.
...
`Active` flag every mounted `Window` maps onto that state (see
[styling.md](styling.md#shared-chrome)).

The bundled themes keep `container` and `window` visually unchanged on hover by
never authoring a `pointerOver` block for either - and, since neither inherits
`control`'s per-state deltas, that is sufficient rather than merely
conventional. The bundled `input` section uses `surface` for its normal face and
`activeControl` during hover (authored directly on `input.pointerOver`), and
`button` also opts into the filled hover background the same way. An external
theme may author an explicit hover contribution on any type that wants it.
...
allowed here because they define the type's own chrome, not something a specific
control instance owns. Individual controls may still set complete local styles
that take precedence over everything a theme supplies.

Twenty-four library controls resolve part or all of their structural `Style`
...
| `chart`                   | chart controls            | `control`      | `axisColor`, `labelColor`, `primaryColor`, `secondaryColor`, `tertiaryColor`, `glyphs`, `fillMode`, `lineMode`                                                                                                                                             |
...
| `menuItem`                | `MenuItem`                | `control`      | `uncheckedGlyph`, `checkedGlyph`, `radioUncheckedGlyph`, `radioCheckedGlyph` (one-character strings)                                                                                                                                                       |
...
| `navigationViewItem`      | `NavigationViewItem`      | `control`      | `idleMarker`, `currentMarker` (one-character strings)                                                                                                                                                                                                      |
| `navigationViewSeparator` | `NavigationViewSeparator` | `control`      | `glyph` (one-character string)                                                                                                                                                                                                                             |
| `table`                   | `Table`                   | `control`      | `glyphs` (three one-character strings), `cellPadding` (`{x, y}` object), `headerForeground`, `headerBackground`, `gridLineColor` (nullable colors - null inherits the table's own face)                                                                    |
...
`"transparent"`/`"default"`, resolved through `Theme.Palette` lazily rather than
an eager parse-time dictionary. A `glyphs` member is a nested object whose own
...
a hand-authored glyph value would be. Every bundled theme except the two
