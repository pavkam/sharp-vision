...
public sealed class CommandTile : Control<CommandTileStyle>
{
    public CommandTile() : base(CommandTileStyle.Definition)
    {
    }
}
```
...
provide the same `Control<TStyle>` facade without surrendering their specialized
behavior. Override `OnStyleChanged` only for genuine post-commit work such as
normalizing an animation phase or projecting an aggregate style onto
heterogeneous retained parts.
...
Implement `CheckedState`, `SelectedState`, or `IndeterminateState` only when the
control genuinely owns that semantic fact. `Focused` means direct focus;
`FocusWithin` means a descendant has focus.
