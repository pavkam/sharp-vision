...
`ScrollBar.Style` is a nullable complete `ScrollBarStyle`. Generated-bar hosts
publish the corresponding nullable `ScrollBarStyle` and always-present
`ActualScrollBarStyle`. Null resolves to the library scrollbar mechanics; the
generated scrollbar itself receives the active semantic control profile. A local
complete styles win. A container copies its effective value to its owned
horizontal and vertical rails, and composite controls such as `TextInput`
...
Normal precedence applies: a local complete style wins over the Theme, which
wins over the code-owned fallback. Partial `ScrollBarStyleSet` values exist only
while completing Theme files.
...
between nested armed containers. This manual ancestry walk stops at an active
[modal plane](modality.md#modal-route-boundaries); no remainder may scroll a
background container. If no in-plane offset changes, the wheel remains unhandled
so the plane's Ignore or Dismiss policy completes it. Keyboard arrows,
PageUp/PageDown, and Home/End drive the same typed commands.
