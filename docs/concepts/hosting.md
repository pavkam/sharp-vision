...
The host then builds the `Application`, wires Ctrl+C to cooperative shutdown
unless `TreatControlCAsInput` is set - through `PosixSignalRegistration` for
`SIGINT` and `SIGQUIT` on Unix, and `Console.CancelKeyPress` on Windows, for the
reason given under [Unix](#unix) - starts the application, waits for completion
or cancellation, stops cleanly, and maps the outcome to a `ConsoleRunStatus`:
`Completed`, `Cancelled`, or `Failed` (when `Application.Failure` is set). The
numeric values remain stable for compatibility: `Redirected=0`, `Completed=1`,
`Cancelled=2`, `Failed=3`, and the appended `UnsupportedTerminal=4`.
