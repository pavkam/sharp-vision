...
suppression from the route's own final `Handled` verdict in one place, so every
consume path is covered uniformly rather than requiring each one to remember to
arm it individually; see [access keys](access-keys.md#overview) for the
adjacent-pair mechanics.
...
chrome or background does not move focus to an application-shell ancestor. Focus
then commits before the pointer event routes. By the time routed handlers run,
the activated Window is already observable through `Application.ActiveWindow`
and `Window.Active`.
...
`PointerDirectlyOver` and every ancestor exposes `PointerOver`. Semantic press
state belongs only to `PressBehavior`; raw pointer dispatch never marks a
...
protected cancellation hook runs next, and the manager-level `Cancelled` event
then publishes the precise `ReleaseReason`. Capture requests made from either
cancellation callback return false until the complete callback sequence has
unwound.
