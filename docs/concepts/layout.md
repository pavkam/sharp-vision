...
Percent tracks resolve against that already spacing-reduced area. `Stack` passes
its pre-spacing axis as an explicit `percentBase` (see
...
override. `TablePresenter.MeasureCells` is shared by both passes and sources the
same candidate from whichever is valid where it is called: the committed
