...
`ControlBase` inheritance for a new primitive leaf. `PressInteractionBase` is
the focusable, press-interactive role for a control whose owned content is
richer than a single caption; `PressableBase` layers the single-text-caption
authoring role on top of it.
...
| Focusable, press-interactive, arbitrary owned content | `PressInteractionBase`                           | Whatever the concrete control needs            |
