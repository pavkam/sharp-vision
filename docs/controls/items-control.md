...
| `OnItemsControlDisposing()`                                                                                                      | `void`                                              | No-op   | Protected virtual; runs before this owner's private presentation host is disposed, so a foreign item owner - `Breadcrumb` and `TabControl` in this library - can settle its own semantic state first. |
