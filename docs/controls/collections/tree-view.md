...
Public offsets, extent, viewport, line increment, page overlap, generated
scrollbar styling, and `ScrollChanged` are inherited from
[`ScrollableCompositeControlBase`](../scrollable-composite-control.md#overview),
which bridges them to the private items container. A source-originated change is
observable on TreeView without exposing its presentation tree.
...
| Inherited `ScrollBars`            | `ScrollBars`                                       | Host-defined                              | Axes enabled by the private items container.                                                                                            |
| Inherited `ShowScrollBars`        | `ShowScrollBars`                                   | Host-defined                              | Reservation policy for generated scrollbars.                                                                                            |
...
| Inherited `Extent`                | `Size`                                             | Layout-dependent                          | Read-only committed content extent of the generated scroll container.                                                                   |
| Inherited `Viewport`              | `Size`                                             | Layout-dependent                          | Read-only committed visible extent of the generated scroll container.                                                                   |
| Inherited `HorizontalOffset`      | `int`                                              | `0`                                       | Valid horizontal content offset; rejects a value outside the current extent.                                                            |
| Inherited `VerticalOffset`        | `int`                                              | `0`                                       | Valid vertical content offset; rejects a value outside the current extent.                                                              |
| Inherited `LineSize`              | `int`                                              | `1`                                       | Non-negative wheel-scroll increment forwarded to the generated scroll container.                                                        |
| Inherited `PageOverlap`           | `int`                                              | `0`                                       | Non-negative cells of context retained between page commands.                                                                           |
| Inherited `ScrollBy(x, y, cause)` | `bool`                                             | —                                         | Scrolls the generated scroll container by signed cell deltas with saturation and endpoint clamping.                                     |
...
| Inherited `ScrollChanged`         | `EventHandler<ScrollChangedEventArgs>`             | No subscribers                            | Forwards the generated scroll container's committed offset changes.                                                                     |
