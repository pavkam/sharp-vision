...
Set `TreeViewItem.Checkable` to display a check mark. Setting `IsChecked` on a
...
var source = new TreeViewItem("src") { Checkable = true };
source.Children.Add(new TreeViewItem("Program.cs") { Checkable = true });
