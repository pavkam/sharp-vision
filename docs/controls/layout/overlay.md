...
`Overlay` owns overlapping managed children with optional absolute positioning
and a deterministic z-order. Unpositioned children share the complete content
box; positioned children resolve their attached offsets against that same box.
...
and has no authored offsets is centered inside the content box. Every arrange
clamps its complete border box after the authored offsets resolve — including
after a resize — without rewriting those offsets. A Window larger than the box
starts at the leading content edge and clips normally. Dragging the title bar
writes the `Left` and `Top` offsets.

Hit testing visits the highest visible z-order first and respects clipping and
`HitTestVisible` pointer transparency. Rendering visits z-order low to high.
