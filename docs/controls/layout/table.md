...
| `SetDataSource<T>(ITableDataSource<T> source, TableRowTemplate<T> rowTemplate, Length rowHeight)` | `void`                                         | —            | Binds this table to a progressive data source with a fixed or viewport-relative uniform row height.                               |
...
//     row => new TableRow([new Text(row.ToString()) { Height = Length.Star(1) }]),
//     rowHeight: Length.Percent(20));
```
...
ever probing a row. Its row height accepts positive `Cells` or `Percent` only;
`Auto`, `Star`, and zero fixed/percentage requests cannot define one uniform
progressive stride and are rejected before mode changes. `SetDataSource` also
requires the table to already be attached to a running dispatcher. Once
progressive, every `Rows`-mutating member (`Rows.Add`/`Insert`/`Remove`/`Clear`,
`SelectRow`, `SelectCell`, `SetSort`) throws `InvalidOperationException`, and
`BeginEdit` simply reports `false` — progressive tables are read-only in this
release. `ClearDataSource()` tears the binding down and returns to an empty
eager table. A rejected `SetDataSource` call always leaves the prior mode —
eager or an earlier progressive source — completely untouched.
...
The progressive row request resolves once per layout transaction. Fixed cells
remain exact; a percentage uses the final scrollbar-aware data viewport after
the optional header is removed, rounds midpoint values away from zero, and
clamps a positive request to one cell for an empty or tiny viewport. The
resolved row stride is the saturating sum of that height and `RowSpacing`.
Windowing, arrangement, hit testing, paging, prefetch margins, cache admission,
and bottom-edge offsets all use that same frozen positive stride with saturating
index and coordinate arithmetic. Resize re-resolves and rewindows atomically,
cancels fetches that left the new window, retains applicable cached rows and
selection keys, and remaps the offset to preserve the logical top row.
