...
| ------------- | ---------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `Phase`       | `double`         | `0`                       | Normalized hue offset in the half-open range `[0, 1)`; rejects a non-finite or out-of-range value.          |
| `CycleLength` | `int`            | `18`                      | Positive terminal-cell divisor for the coordinate-to-hue calculation; rejects zero or a negative value.     |
| `Direction`   | `PrismDirection` | `PrismDirection.Diagonal` | Selects the horizontal, vertical, or diagonal content-relative coordinate axis; rejects an undefined value. |
...
- The `PrismTests` fixture demonstrates the defaults; that non-finite,
