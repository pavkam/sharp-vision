...
| Inherited `Border`                                     | Window profile            | Configures the complete frame.                                          |
| Inherited `Face`                                       | Window profile            | Paints the theme-defined opaque `windowSurface` body.                   |
...
| `Active`                                               | `false`, read-only        | Reports whether this is the owning Application's active Window.         |
...
any kind while the mark was a control property.
...
`Application.ActiveWindow`, and the matching Window reports `Active`. A
...
activation clear. The default Window profile maps `Active` onto its existing
`FocusWithin` appearance contribution, changing only the frame foreground to
`SemanticColor.ActiveBorder`. `ContainsFocus` and `Focused` keep their
