...
| `CancelDrag(bool releaseCapture)`                                                                                                                              | `void`                                        | —                 | Protected; cancels an active drag, if `EnableDrag` was called.                                                                                                                                                                                                                                                                                                             |
...
`Slider`, `ColorPlane`, `SplitPane`, and `ScrollBar` compose `EnableDrag` the
same way for pointer-driven value editing and divider resizing: a drag starts
from a pointer press inside the enabled bounds, requests focus, and acquires
pointer capture through `tryCapture`; `TryFocusForInteraction`,
`CanContinueAfterFocus`, and `TryStartDrag` share the same "request focus, then
verify the interaction may still proceed" guard that protects a synchronous
focus handler from invalidating the control before the gesture continues.
