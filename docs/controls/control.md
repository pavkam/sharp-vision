...
| `EnablePressActivation(Func<Rect>?, Action<ActivationCause>?, Func<bool>?, Func<bool>?, Func<bool>?)`                                                          | `void`                                        | —                 | Protected; opts into the shared pointer-press and Enter/Space keyboard-activation state machine. Every parameter is optional and defaults to this control's own bounds, `Activate`, availability, Space-completion, and focus request; a control supplies one only where its interaction genuinely differs. Throws `InvalidOperationException` if called twice.                                                                                                                                                                     |
| `HandlePressActivation(RoutedEventArgs)`                                                                                                                       | `void`                                        | —                 | Protected; routes one event through the press-activation state machine; a no-op before `EnablePressActivation` runs.                                                                                                                                                                                                                                                                                                                                                                                                                |
...
`Expander`, `ListItem`, `Pager`, and `NavigationViewGroup` do; see
[the pressable capabilities page](pressable.md#overview) for the shared
pointer/keyboard press state machine and its full interaction contract.
`Slider`, `ColorPlane`, `SplitPane`, `ScrollBar`, and `Window` compose
`EnableDrag` the same way for pointer-driven value editing, divider resizing,
and title-bar move/corner resize: a drag starts from a pointer press inside the
enabled bounds, requests focus, and acquires pointer capture through
