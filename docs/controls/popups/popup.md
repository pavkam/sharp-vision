...
| `ResolvedPlacement`                                                                            | `PopupPlacement`                               | `Below`                         | Read-only; the placement the latest arranged open presentation actually resolved.                                                                                                                                                          |
| `ConstrainToRoot`                                                                              | `bool`                                         | `true`                          | Whether placement flips and clamps inside the owning root; a composite owner that arranges its own popup child sets this false.                                                                                                            |
| `FixedOrigin`                                                                                  | `Point?`                                       | `null`                          | An optional fixed origin that overrides anchor-based positioning; a context menu opened at a pointer position sets this instead of assigning `Anchor`.                                                                                     |
| `TracksAnchorReflow`                                                                           | `bool`                                         | `true`                          | Whether this popup re-resolves placement when its foreign `Anchor` reflows while open; a self-anchored composite that re-arranges its own popup child every pass - a submenu, a ComboBox drop-down - sets this false before first opening. |
...
| Protected internal `ConfigureLightDismiss(PopupLightDismissPolicy policy)`                     | `void`                                         | —                               | Setup, called once from a derived family's constructor; opts the Popup into outside-press dismissal.                                                                                                                                       |
| Protected virtual `CompleteOpenContent(bool suppressFocusOnOpen)`                              | `bool`                                         | —                               | Completes content visibility, focus, and family setup for one opening transaction.                                                                                                                                                         |
| Protected virtual `OnContentAvailable()`                                                       | `bool`                                         | —                               | Completes family-specific setup after current content becomes available during opening.                                                                                                                                                    |
| Protected virtual `OnAnchorReflow()`                                                           | `void`                                         | —                               | Responds to the foreign anchor reflowing while open; the default re-resolves placement.                                                                                                                                                    |
| Protected `ExcludePopupPeers(isFamily, isExcluded, continuation)`                              | `bool`                                         | —                               | Closes eligible Popup-family peers around one callback-bearing opening stage.                                                                                                                                                              |
| Protected `LayoutAgainstRoot()`                                                                | `void`                                         | —                               | Forces immediate placement resolution against the current root bounds.                                                                                                                                                                     |
| Protected internal `CloseImmediatelyForPeerTransition()`                                       | `FloatingSurfaceCloseOutcome`                  | —                               | Closes this Popup synchronously, bypassing fade-out, for internal peer and submenu replacement.                                                                                                                                            |
...
| `ContentDisposalRequested`                                                                     | `EventHandler<OwnedContentDisposalEventArgs>`  | —                               | Raised before directly disposed content leaves this popup's owned slot; a menu-like owner that tracks its own popup content - `ContextMenu` and `MenuItem`'s submenu, in this library - subscribes to detach its own bookkeeping first.    |
...
`PopupLightDismissPolicy` is the public immutable value type
`ConfigureLightDismiss` accepts. Its constructor validates a non-empty `Buttons`
set and a non-null `Dismiss` callback, and its four read-only properties -
`IncludeAnchor`, `Buttons`, `InterceptAtModalBoundary`, and `Dismiss` -
describe, respectively, whether the current `Anchor` counts as inside the
dismissal surface, which pointer buttons dismiss, whether dismissal participates
at a modal boundary, and the callback that requests family-specific closure.
`Flyout` and `ContextMenu` both build one from their own constructors; an
externally defined family builds its own the same way.

`OwnedContentDisposalEventArgs` is the public sealed argument type
`ContentDisposalRequested` carries. Its one read-only `Content` property exposes
the owned control whose direct disposal was requested; only `Popup` itself
