...
a replaceable semantic content value, and use `Container` only for a genuine
layout control whose arbitrary children are part of its public contract.
...
rendering, normal and popup hit testing, focus navigation, and compilation from
the unfriended consumer assembly.
