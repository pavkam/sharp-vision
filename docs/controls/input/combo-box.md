...
| `Opened`                                 | `false`             | Controls popup layout, rendering, and hit testing.                                                             |
...
| `PopupChrome`, `ResetPopupChrome`        | `default`           | Overrides or resets the private Popup's border and shadow together (see [Popup](../popups/popup.md#overview)). |
| Themed disclosure glyph                  | `InputStyle` (`▼`)  | Authored once for every drop-down input via a theme's `styles.input` section.                                  |
...
- `Opened` controls Popup layout, rendering, hit testing, and one
