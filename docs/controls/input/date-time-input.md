...
| Inherited `DropDownHeight`                   | `Length`                                                | `Length.Cells(10)`               | The automatic, fixed-cell, or placement-relative maximum visible calendar height.                                                           |
...
| Inherited `PopupChrome`                      | `PopupChrome`                                           | `default`                        | Overrides the owned Calendar popup's border and shadow together.                                                                            |
| Inherited `ResetPopupChrome()`               | `void`                                                  | —                                | Returns the Calendar popup's border and shadow to `PopupChrome` ownership.                                                                  |
...
| Inherited `DropDownOpened`, `DropDownClosed` | `EventHandler`                                          | no subscribers                   | Raised after the Calendar popup opens or closes.                                                                                            |
...
properties: `DateTimeInput` raises its own `PropertyChanged` for one of them
both when its own setter commits the change and when the owned popup or Calendar
...
last segment, Home or End when already there, Delete or Backspace over an empty
value, and a repeated A or P that only moves the designator highlight. The key
is the field's own, so a bounded field inside a scrolling or directionally
navigating container never scrolls or moves focus in that container.
