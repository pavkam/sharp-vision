...
| `ThreeState`                   | `false`        | Selects the two-state or three-state activation cycle.         |
...
`CheckBoxGlyphs`, and the inherited `Face`/`Border`/`Shadow`. A `with`
expression creates a validated member-wise copy of `CheckBoxStyle.Default`. A
theme document may additionally author a `styles.checkBox` section with a
`markStyle` string member (`"square"`, `"brackets"`, or `"tick"`); an active
theme's section supplies `MarkStyle` ahead of the code-owned default whenever no
local `Style` is assigned (see
...
and assigning `null` restores it. Every glyph is printable and one cell wide
under the normal width policy. The brackets style reserves three cells; the
other styles reserve one.
