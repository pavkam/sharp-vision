...
`RadioButtonGlyphs`, and the inherited `Face`/`Border`/`Shadow`. A `with`
expression creates a validated member-wise copy of `RadioButtonStyle.Default` or
of any resolved style. A theme document may additionally author a
`styles.radioButton` section with a `markStyle` string member (`"circle"` or
`"parentheses"`); an active theme's section supplies `MarkStyle` ahead of the
code-owned default whenever no local `Style` is assigned (see
[themes.md](../../concepts/themes.md#style-types)). The glyph pair remains
