...
expression creates a validated member-wise copy of `RadioButtonStyle.Default`
(`Parentheses`) or of any resolved style. RadioButton declares no `styles.*`
theme key of its own: its code-owned mark style and glyph pair come from the
active theme's root-level `glyphs` field whenever no local `Style` is assigned
(see [themes.md](../../concepts/themes.md#glyph-families)). Assigning `Style`
...
| Home           | Moves focus and selection to the first eligible group member.                     |
| End            | Moves focus and selection to the last eligible group member.                      |
| Alt+access key | Focuses and selects the radio button when `Text` declares that access key.        |

The arrow and endpoint keys follow the shared
[keyboard modifier policy](../../concepts/input-routing.md#keyboard-modifier-policy)
for scalar navigation: Caps Lock and Num Lock ride along, while a navigation key
...
keeps walking the group and re-raising the selection events. Home and End select
the first or last eligible member without wrapping. Disabled, hidden, collapsed,
detached, and non-focusable members are skipped. Disabled styling wins even when
a retained member remains selected.
...
- Arrow, Home, and End keys move the selection while skipping unavailable
  members, and unnamed and named scopes group exactly as described.
...
- Both mark layouts render correctly at either caption edge and at every valid
  mark gap, Unicode captions stay owned by their member, and rendering produces
  the exact terminal rows.
