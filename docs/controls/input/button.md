...
| `IsDefault`                   | `bool`                              | `false`                    | Whether an owning Window treats Enter as a fallback activation for this button. Several buttons may keep this flag at once; only the one the owning Window actually resolves - the first enabled, visible candidate in ownership order - presents the `Current` visual state a theme's `styles.button.current` can mark. A button with no owning Window presents `Current` while it is simply enabled and visible. |
