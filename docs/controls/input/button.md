...
| `IsDefault`                   | `bool`                              | `false`                    | Whether an owning Window treats Enter as a fallback activation for this button; also presents it in the `Current` visual state so a theme's `styles.button.current` can mark it. |
