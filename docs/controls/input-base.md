...
| `CaptureCommand()`                                           | `CommandBinding`         | —       | Protected; captures `Command` and `CommandParameter` as one immutable snapshot before an activation publishes any callback, so reentrant rebinding or disposal during that callback cannot redirect work the activation already accepted. `CheckBox`, `RadioButton`, `MenuItem`, `NavigationViewItem`, `BreadcrumbItem`, `CommandBar`, and `CommandBarItem` all capture before publishing their own activation event.                                                                                                                                                                                                                                                                                 |
| `ExecuteCommandIfAny(CommandBinding binding)`                | `void`                   | —       | Protected static; invokes one previously captured binding when it allows execution - the counterpart to `ExecuteCommandIfAny()` for a control that must capture before publishing a callback and execute afterward. Equivalent to calling `binding.ExecuteIfAny()` directly.                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `IsCommandExecutable`                                        | `bool`                   | —       | Protected; whether `Command` is unbound or reports it can currently execute with `CommandParameter`. A control that presents its disabled appearance while a bound command cannot execute reads this from its `GetAppearanceState()` override, the way `Button` and `HyperlinkButton` do; not every `InputBase` ties its appearance to command executability.                                                                                                                                                                                                                                                                                                                                         |
...
A concrete activation implementation that publishes its own event before
executing the command follows the capture-then-execute pattern: call
`CaptureCommand()` first, publish the activation callback, then execute the
returned `CommandBinding` (through `ExecuteCommandIfAny(CommandBinding)`, or
directly through the binding's own `CanExecute()`/`ExecuteIfAny()`). Capturing
before the callback runs means a callback that rebinds `Command` or disposes the
control cannot redirect work the activation transaction already accepted.
`CommandBinding` is a public immutable value; a control that owns a sibling
collaborator not itself derived from `InputBase` - `CommandBar` reading a
`CommandBarItem`'s binding, or `Breadcrumb` reading a `BreadcrumbItem`'s - goes
through a small `internal` bridge method the item exposes for its owner, since
`CaptureCommand` itself is protected for a derived control's own use.
