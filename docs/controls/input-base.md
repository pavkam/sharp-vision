...
| `EnablePopup(...)`                                           | `Popup`                  | —        | Opts into an owner-managed popup that preserves unrelated popup planes and open state across temporary ancestor unavailability, registers its framework-part slot, and composes the shared open/close coordinator. |
