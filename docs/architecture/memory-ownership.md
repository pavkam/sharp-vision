...
`ContentControl` registers the capacity-one public content role. `InputBase`'s
`EnableCaption` capability does not inherit `ContentControl`: it registers its
own capacity-one slot for a lazily materialized owned caption child, reachable
only through the `Text` string property, never as `Children`. `ComboBox`
registers exactly one private popup-layer framework-part slot, and
...
disposal and serializes writes and flushes through one gate. Disposal closes
admission and cancels a shared disposal token, which also unblocks a writer or
flusher still waiting on the gate rather than leaving it queued behind a holder
that never releases. The subsequent wait for every operation admitted before
that boundary to leave the gate is bounded by a write drain timeout (the
`writeDrainTimeout` constructor parameter, defaulting to one second): an
operation that neither completes nor honors cancellation within that budget is
abandoned instead of disposal hanging on it. Only then does disposal dispose the
gate and owned streams. Stream ownership is decided per stream: the `leaveOpen`
overload applies one decision to both streams, while
