...
cannot grow work without bound.
...
lease and transport/resize source, resolve and validate one terminal profile,
negotiate terminal capabilities, map Ctrl+C to cooperative shutdown unless
`TreatControlCAsInput` is set, attach the supplied `Screen`, start the
application, wait for completion or cancellation, stop cleanly, and restore host
terminal state.
...
constructs no application or session and therefore emits no mode, query, or
renderer bytes.
...
unrestricted pointer values otherwise use capture then hit testing. Terminal
focus loss cancels pointer interaction. Each input drain processes resulting
layout/render invalidation before idleness. Resize preserves the exact managers,
active scope, and eligible focused target.

## Out-of-band protocol writes

Implemented output protocols — described bell, proven `SetTitle`, and
...
mutate the terminal cell grid, cursor, or rendition state, so successful service
writes do not invalidate the renderer front frame. The described title prefix
and suffix are both expanded before the UTF-8 payload is queued; incomplete or
failed pairs publish no bytes. Control-bearing title payloads are rejected
before either expansion or queueing. A failed out-of-band transport write is an
application failure and stops the session; no later frame assumes that transport
remains usable.
...
even if paired with a constructed `Supported` state. Alternate screen and cursor
policy are explicit non-detected options, but output requires complete
expandable `smcup`/`rmcup` and `civis`/`cnorm` description pairs. A complete
`smkx`/`rmkx` pair is leased only when the owned key map contains an SS3
application cursor, Home, or End spelling; SS3 F1–F4 alone does not qualify.
Each attempted enable becomes a lease owning exact enable and disable bytes
before transport I/O, so even an uncertain partial write receives its exact
conservative cleanup attempt.
...
cancellation, or handler exception. Lifecycle programs expand with one
