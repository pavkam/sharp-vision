...
The restriction is scoped to the session whose own run raised the callback.
Disposing a _different_ session from inside a callback is legal, and does not
wait on the running session's own completion.
