...
/// it — the hosting test process itself cannot self-attach to a ConPTY it creates (see issue
/// #35). This probe is that attached process: it exercises the public
...
internal static class ConPtyProbe
{
    ...
    private static async Task RunDisposeTwiceAsync()
    {
        ...
        // once (see ConsoleConnection.DisposeAsync and issue #35 item 4).
        await connection.DisposeAsync().ConfigureAwait(false);
    ...
    private static async Task RunCancelledAsync()
    {
        ...
        // the saved modes (issue #35 approved test:
