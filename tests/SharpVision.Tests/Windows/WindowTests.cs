...
public sealed class WindowTests
{
    ...
    public void Type_WhenInspected_DerivesFromFloatingSurfaceWithoutRoleOrChildCollection()
    {
        ...
        type.BaseType.ShouldBe(typeof(ChromeAuthoringFloatingSurface));
    ...
    public void Dispose_WhenWindowOwnsReplacement_DisposesCurrentOnceAndPublishesCommittedClear()
    {
        ...
        window.PropertyChanged += (_, eventArgs) =>
        {
            if (eventArgs.PropertyName == nameof(ContentControl.Content))
            {
                observations.Add((window.Content, current.Parent, current.Disposed, current.DisposingCalls));
            }
        };
    ...
    /// <summary>Verifies a never-attached, still-Visible Window - the only shape the ordinary
    /// two-clause guard cannot distinguish from "legitimately still open", since it has no
    /// SurfacePresented transition to collapse or detect a repeat from - raises CloseRequested
    ...
    /// <summary>Verifies Visibility returning to Visible after a never-attached close reopens the
