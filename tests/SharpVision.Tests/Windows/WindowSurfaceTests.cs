...
public sealed class WindowSurfaceTests
{
    ...
    {
        // Arrange
        var themeA = WithColor(SemanticColor.FocusedText, Color.Rgb(10, 20, 30));
        var themeB = WithColor(SemanticColor.FocusedText, Color.Rgb(200, 210, 220));
        var window = new Window
        {
            CanClose = true,
            Width = Length.Cells(12),
    ...
    public async Task Resize_WhenPointerCaptureIsLost_CancelsGestureAsync()
    {
        ...
        await surface.UpdateAsync(() => window.Enabled = false, "disable Window mid-resize");

        // Assert — capture released, and a further move under the stale press does nothing
        surface.ShouldHaveCapture(null);
        var sizeAfterCancellation = window.Bounds;
        await surface.UpdateAsync(() => window.Enabled = true, "re-enable Window");
    ...
    public async Task Input_WhenWindowHostsButtons_RoutesFallbacksAndCleansFocusAsync()
    {
        ...
        window.PointerOver.ShouldBeTrue();
        window.PointerDirectlyOver.ShouldBeFalse();
        window.Focused.ShouldBeFalse();
        window.Pressed.ShouldBeFalse();
        ...
        // Act unavailable
        await surface.UpdateAsync(() => window.Enabled = false, "disable focused Window");

        // Assert cleanup
        accept.Focused.ShouldBeFalse();
        window.Pressed.ShouldBeFalse();
