...
public sealed class FloatingSurfaceBaseConsumerTests
{
    /// <summary>Verifies an external package consumer can run a derived surface through its protected lifecycle.</summary>
    [Fact]
    public async Task FloatingSurface_WhenDerivedFromPackedPackage_CanRunProtectedLifecycleAsync()
    {
        ...
        var fixtureRoot = Path.Combine(
            ...
            "Compatibility",
            "PackageConsumers",
            "FloatingSurfaceConsumer");
        var temporaryRoot = Path.Combine(
            Path.GetTempPath(),
            $"sharpvision-floating-surface-consumer-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            await RunDotnetAsync(
                ...
                cancellationToken,
                "build",
                "SharpVision.FloatingSurfaceConsumer.csproj",
                "--configuration",
                "Release",
            ...
            await RunDotnetAsync(
                consumerRoot,
                cancellationToken,
                Path.Combine("bin", "Release", "net10.0", "SharpVision.FloatingSurfaceConsumer.dll"));
        }
        finally
        {
