...
public sealed partial class ModalityManagerTests
{
    ...
    private static void RunCase(Dispatcher dispatcher, int seed, int sample)
    {
        ...
        {
            handle.Active.ShouldBeFalse(Context(_stepCount));
        }
        ...
        {
            Record(operation);
            var followsInactiveHandle = handles.Exists(static handle => !handle.Active);
        ...
        void Dispatch(ControlBase pointOwner, PointerAction action, string operation)
        {
            ...
            if (expectsDismissal)
            {
                ...
                activeBefore.ShouldNotBeNull().Active.ShouldBeFalse(Context(-1));
        ...
        void AssertInvariants(int step)
        {
            ...
            foreach (var handle in handles)
            {
                ...
                handle.Active.ShouldBe(expectedActive, context);
            ...
            {
                handle.Active.ShouldBeFalse(context);
            }
