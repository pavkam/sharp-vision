...
public sealed class TemporalSegmentTests
{
    ...
    /// <summary>Verifies an editable descriptor rejects digit capacities outside the supported
    /// zero-through-seven custom-format range.</summary>
    [Theory]
    [InlineData(-2)]
    [InlineData(8)]
    ...
    {
        _ = Should.Throw<ArgumentOutOfRangeException>(() => new PatternSegment(TemporalSegmentKind.Day, 0, 'd'));
        _ = Should.Throw<ArgumentOutOfRangeException>(() => new PatternSegment(TemporalSegmentKind.Day, -1, 'd'));
        _ = Should.Throw<ArgumentOutOfRangeException>(() => new PatternSegment(TemporalSegmentKind.Day, 1, '\0'));
        _ = Should.Throw<ArgumentNullException>(() => new PatternSegment(null!));
    }

    /// <summary>Verifies pattern-segment equality compares literal text, kind, run length, and
    /// case-sensitive pattern character.</summary>
    [Fact]
    public void PatternSegment_Equality_ComparesEveryMember()
    {
        var run = new PatternSegment(TemporalSegmentKind.Year, 4, 'y');
        var sameRun = new PatternSegment(TemporalSegmentKind.Year, 4, 'y');
        var shorterRun = new PatternSegment(TemporalSegmentKind.Year, 2, 'y');
        var otherPatternCharacter = new PatternSegment(TemporalSegmentKind.Year, 4, 'Y');
        ...
        (run != otherPatternCharacter).ShouldBeTrue();
    ...
    public void ParseTokens_WhenTokensArePercentPrefixed_ProducesSingleLetterRuns()
    {
        ...
        tokens.ShouldBe(
        [
            new PatternSegment(TemporalSegmentKind.Month, 1, 'M'),
            new PatternSegment("/"),
            new PatternSegment(TemporalSegmentKind.Day, 1, 'd'),
            new PatternSegment("/"),
            new PatternSegment(TemporalSegmentKind.Year, 4, 'y')
        ]);
    }
    ...
    public void ParseTokens_WhenPercentIsTrailing_DropsIt()
    {
        ...
        tokens.ShouldBe([new PatternSegment(TemporalSegmentKind.Day, 2, 'd')]);
    }
    ...
    public void ParseTokens_WhenQuotedLiteralEscapesQuote_KeepsQuoteAndMergesAdjacentLiterals()
    {
        ...
        [
            ...
            new PatternSegment(TemporalSegmentKind.Day, 2, 'd')
        ]);
    }
    ...
    public void ParseTokens_WhenQuotedLiteralIsEmptyOrUnterminated_HandlesBothWithoutThrowing()
    {
        ...
        empty.ShouldBe([new PatternSegment(TemporalSegmentKind.Day, 2, 'd')]);
