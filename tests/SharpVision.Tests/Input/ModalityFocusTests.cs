...
public sealed class ModalityFocusTests
{
    ...
    public async Task Focus_WhenTargetIsOutsideActivePlane_RejectsWithoutCallbacksOrMutationAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            var inside = new ProbeControl { Focusable = true };
    ...
    public async Task Enter_WhenChangingInvalidatesInitialFocus_UsesValidFallbackAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            var plane = new ProbeContainer();
            var initial = new ProbeControl { Focusable = true };
            var fallback = new ProbeControl { Focusable = true };
            plane.Children.Add(initial);
    ...
    public async Task Enter_WhenQueuedFocusCallbackThrows_RollsBackCompleteEntryStackAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            var root = new ProbeContainer();
            var background = new ProbeControl { Focusable = true };
            var requested = new ProbeControl { Focusable = true };
            var plane = new ProbeContainer();
            var initial = new ProbeControl { Focusable = true };
            var youngerRoot = new ProbeContainer();
            var youngerFocus = new ProbeControl { Focusable = true };
            ...
            focus.Changing += (_, eventArgs) =>
            {
                if (scope is null && ReferenceEquals(eventArgs.Next, requested))
                {
                    ...
                    scope.Exited += (_, _) =>
                    {
                        ...
                        modality.Active.ShouldBeNull();
                        focus.Focused.ShouldBeSameAs(background);
                        background.Focused.ShouldBeTrue();
                        initial.Focused.ShouldBeFalse();
                        outerTracker = false;
                        exitOrder.Add("outer");
                    };
                }
                else if (younger is null && ReferenceEquals(eventArgs.Next, initial))
                {
                    ...
                    younger.Exited += (_, _) =>
                    {
                        youngerWasInactiveDuringYoungerExit = !younger.Active;
                        outerWasInactiveDuringYoungerExit = !scope.ShouldNotBeNull().Active;
                        activeDuringYoungerExit = modality.Active;
                        focusDuringYoungerExit = focus.Focused;
                        requestedWasFocusedDuringYoungerExit = requested.Focused;
                        initialWasFocusedDuringYoungerExit = initial.Focused;
                        youngerWasFocusedDuringYoungerExit = youngerFocus.Focused;
            ...
            failed.Active.ShouldBeFalse();
            failedYounger.Active.ShouldBeFalse();
            modality.Active.ShouldBeNull();
            focus.Focused.ShouldBeSameAs(background);
            requested.Focused.ShouldBeFalse();
            initial.Focused.ShouldBeFalse();
            youngerFocus.Focused.ShouldBeFalse();
    ...
    public async Task Focus_WhenOrdinaryRequestOutlivesRolledBackEntry_RejectsStaleScopeContextAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            var trigger = new ProbeControl { Focusable = true };
            var parentAfter = new ProbeControl { Focusable = true };
            var childRoot = new ProbeContainer();
            var childInitial = new ProbeControl { Focusable = true };
            var childQueued = new ProbeControl { Focusable = true };
            ...
            childQueued.Focused.ShouldBeFalse();
    ...
    public async Task Dispose_WhenRestoreChangingThrows_RepairsFocusBeforeExitedAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            inner.Exited += (_, _) =>
            {
                ...
                focus.Focused.ShouldBeSameAs(outerFocus);
                outerFocus.Focused.ShouldBeTrue();
                innerFocus.Focused.ShouldBeFalse();
                order.Add("exited");
                throw later;
            };
        ...
        string callback)
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            var innerRoot = new ProbeContainer();
            var innerFocus = new ProbeControl { Focusable = true };
            var nextInner = new ProbeControl { Focusable = true };
            outerRoot.Children.Add(outerFocus);
            outerRoot.Children.Add(innerRoot);
            innerRoot.Children.Add(innerFocus);
            ...
            inner.Exited += (_, _) =>
            {
                ...
                outerFocus.Focused.ShouldBeTrue();
                innerFocus.Focused.ShouldBeFalse();
                nextInner.Focused.ShouldBeFalse();
                order.Add("exited");
            };
    ...
    public async Task Dispose_WhenOlderScopeEndsFromFocusCallback_CommitsBatchBeforeReturningAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            focus.Focused.ShouldBeSameAs(background);
            background.Focused.ShouldBeTrue();
            outerFocus.Focused.ShouldBeFalse();
            innerFocus.Focused.ShouldBeFalse();
            trigger.Focused.ShouldBeFalse();
        }, TestContext.Current.CancellationToken);
    }
    ...
    public async Task Dispose_WhenFocusCleanupFailsAfterDeferredUnwind_PreservesEarlierFailureAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            var parentFocus = new ProbeControl { Focusable = true };
            var childRoot = new ProbeContainer();
            var childFocus = new ProbeControl { Focusable = true };
            var trigger = new ProbeControl { Focusable = true };
            parentRoot.Children.Add(parentFocus);
            ...
            {
                if (eventArgs.PropertyName == nameof(ControlBase.Focused) && !childFocus.Focused)
                {
                    disposalFailures++;
            ...
            child.Active.ShouldBeFalse();
            parent.Active.ShouldBeTrue();
            modality.Active.ShouldBeSameAs(parent);
            pointer.Captured.ShouldBeNull();
            focus.Focused.ShouldBeNull();
            parentFocus.Focused.ShouldBeFalse();
            childFocus.Focused.ShouldBeFalse();
            trigger.Focused.ShouldBeFalse();
            root.FocusOwner.ShouldBeNull();

            modality.Shutdown();
            parent.Active.ShouldBeFalse();
            modality.Active.ShouldBeNull();
        }, TestContext.Current.CancellationToken);
    }
    ...
    public async Task Dispose_WhenEnclosingFocusCallbackFailsLater_PreservesDeferredUnwindFailureAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            parentFocus.Focused.ShouldBeTrue();
            childFocus.Focused.ShouldBeFalse();
            trigger.Focused.ShouldBeFalse();
        ...
        string callback)
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            void EnterReentrant()
            {
                ...
                {
                    reentrantWasInactiveDuringExit = !reentrant.Active;
                    order.Add("reentrant");
                };
            }
            ...
            requested.Active.ShouldBeFalse();
            reentrant.ShouldNotBeNull().Active.ShouldBeFalse();
    ...
    public async Task Enter_WhenQueuedEntryFocusIsCanceledByFocusDisposal_RollsBackScopeAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            rolledBack.Active.ShouldBeFalse();
            modality.Active.ShouldBeNull();
            exited.ShouldBe(1);
            focus.Focused.ShouldBeNull();
            background.Focused.ShouldBeFalse();
    ...
    public async Task Dispose_WhenSavedTargetIsUnavailable_UsesParentPlaneFallbackAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            var outerRoot = new ProbeContainer();
            var saved = new ProbeControl { Focusable = true };
            var fallback = new ProbeControl { Focusable = true };
            var innerRoot = new ProbeContainer();
            var innerFocus = new ProbeControl { Focusable = true };
    ...
    public async Task Dispose_WhenSavedApplicationTargetIsUnavailable_ClearsFocusAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            var scope = modality.Enter(plane, initialFocus: initial);
            saved.Focusable = false;

            scope.Dispose();

            focus.Focused.ShouldBeNull();
            unrelated.Focused.ShouldBeFalse();
        }, TestContext.Current.CancellationToken);
    }
