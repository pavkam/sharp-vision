...
/// <summary>Verifies Theme.ResolveSectionColor resolves a registrable section's color string
/// the same way ThemeCatalog.cs resolves a profile color, but through the public Palette instead of the
/// private parse-time dictionary.</summary>
public sealed class ThemeSectionColorResolutionTests
{
    ...
    [Fact]
    public void ResolveSectionColor_WhenValueIsHexLiteral_ResolvesLiteral()
    {
        var theme = ThemeCatalog.Parse(ThemeJson.Create());

        var value = theme.ResolveSectionColor("#123456", "context");
    ...
    [Fact]
    public void ResolveSectionColor_WhenHexLiteralIsInvalid_Throws()
    {
        var theme = ThemeCatalog.Parse(ThemeJson.Create());

        _ = Should.Throw<InvalidDataException>(() => theme.ResolveSectionColor("#zzz", "context"));
    }
