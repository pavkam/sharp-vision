...
public sealed class DisplayPanelTests
{
        ...
        bool hasCanvasText)
    {
        ...
        if (expectedLabel == "界")
        {
            frame.GetCell(new Point(2, 1)).Continuation.ShouldBeTrue();
        }
    ...
    private static Grid Create(out ControlText label, out Overlay canvas)
    {
        var root = new Grid();
        ...
        Overlay.SetLeft(canvasText, Length.Cells(1));
