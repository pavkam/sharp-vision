...
public sealed class DisplayPanelTests
{
    ...
    public async Task Render_WhenDisplayPanelsCompose_PreservesGeometryCellsAndDamageAsync()
    {
        ...
        Bytes(terminal).AsSpan().IndexOf("界"u8)
    ...
    private static Grid Create(out ControlText label, out ControlOverlay canvas)
    {
        var root = new Grid();
        ...
        var overlay = new ControlOverlay { Height = Length.Cells(2) };
        overlay.Children.Add(new ControlText("O"));
        stack.Children.Add(overlay);
        canvas = new ControlOverlay();
        var canvasText = new ControlText("C");
        ControlOverlay.SetLeft(canvasText, Length.Cells(1));
