...
using ControlCalendar = UiCalendar;
...
public sealed class BindingExtensionTests
{
    ...
    {
        var model = new BindingModel { Expanded = false };
        var expander = new Expander();
        using var binding = expander.Bind(model, source => source.Expanded);
    ...
    {
        var model = new BindingModel { Expanded = true };
        var expander = new Expander();
        using var binding = expander.Bind(model, source => source.Expanded);

        expander.Expanded = false;

        model.Expanded.ShouldBeFalse();
    }
