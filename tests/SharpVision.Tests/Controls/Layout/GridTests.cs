...
public sealed class GridTests
{
    ...
    public void Enabled_WhenToggled_CascadesEffectiveIsEnabledToOwnedChild()
    {
        ...
        grid.EffectiveIsEnabled.ShouldBeFalse();
        child.EffectiveIsEnabled.ShouldBeFalse();

        grid.IsEnabled = true;

        grid.EffectiveIsEnabled.ShouldBeTrue();
        child.EffectiveIsEnabled.ShouldBeTrue();
    }

    /// <summary>Verifies an Auto column absorbs an AutoSize scrollable child's clamped
    /// DesiredSize, not the unclamped value a scrollbar reservation would otherwise report,
    /// so the column never reserves a cell wider than the child ever actually uses.</summary>
    [Fact]
    public void Grid_WhenAutoSizeChildScrollbarReservationClampsAtMaxWidth_AutoColumnMatchesChildBounds()
    {
        ...
        new LayoutEngine().Layout(grid, new Size(40, 40));

        grid.DesiredSize.Width.ShouldBe(container.Bounds.Width);
        grid.Bounds.Width.ShouldBe(container.Bounds.Width);
    }
}
