...
public sealed class GroupBoxSurfaceTests
{
    ...
    /// <summary>Verifies a disabled group keeps its unavailable caption color while retaining mnemonic syntax.</summary>
    [Fact]
    ...
    public async Task Pointer_WhenContentIsHovered_TracksComposedAncestryWithoutInteractionAsync()
    {
        ...
        group.PointerDirectlyOver.ShouldBeFalse();
        content.PointerDirectlyOver.ShouldBeTrue();
        group.Focused.ShouldBeFalse();
        group.Pressed.ShouldBeFalse();
        content.Focused.ShouldBeFalse();
    }
