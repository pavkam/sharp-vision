...
public sealed class ComponentRoleTests
{
    ...
    /// <summary>Verifies every text-captioned InputBase descendant uses the single-text-caption
    /// authoring role: Content is structurally absent (not merely hidden), and the only caption
    /// surface is the string <see cref="InputBase.Text"/> property.</summary>
    [Fact]
    public void Type_WhenInspected_UsesSingleTextCaptionRole()
    {
        var inputBase = typeof(InputBase);
        Type[] concrete =
        [
            typeof(Button),
            typeof(CheckBox),
        ...
        inputBase.BaseType.ShouldBe(typeof(ControlBase));
        inputBase.GetProperty(nameof(ContentControl.Content),
                BindingFlags.Public | BindingFlags.Instance)
            .ShouldBeNull();
        inputBase.GetProperty("Children", BindingFlags.Public | BindingFlags.Instance).ShouldBeNull();
        inputBase.GetProperty(nameof(InputBase.Text), BindingFlags.Public | BindingFlags.Instance)
            .ShouldNotBeNull()
            .PropertyType.ShouldBe(typeof(string));
        inputBase.GetConstructors(BindingFlags.NonPublic | BindingFlags.Instance)
            .ShouldAllBe(constructor => constructor.GetParameters().Length == 0);
        inputBase.GetFields(BindingFlags.NonPublic | BindingFlags.Instance)
            .ShouldNotContain(field => field.FieldType == typeof(PointerManager));

        foreach (var type in concrete)
        {
            typeof(InputBase).IsAssignableFrom(type).ShouldBeTrue(type.FullName);
    ...
    /// <summary>Verifies ListItem composes press behavior directly instead of opting into
    /// InputBase's caption capability, so it keeps arbitrary owned <see cref="ContentControl.Content"/>
    /// for realized template output, which the single-text-caption capability does not support.</summary>
    [Fact]
    public void ListItem_WhenInspected_ComposesPressBehaviorInsteadOfDerivingInputBase()
    {
        var type = typeof(ListItem);

        typeof(InputBase).IsAssignableFrom(type).ShouldBeFalse();
    ...
    public void ComboBox_WhenInspected_IsDirectControlWithoutLeakedPartsOrHiddenMembers()
    {
        ...
        type.BaseType.ShouldBe(typeof(InputBase));
        ...
        type.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly)
            .Any(property => property.PropertyType == typeof(Popup) ||
                             property.PropertyType == typeof(UiListView))
            .ShouldBeFalse();
    }
    ...
    public void MenuSeparator_WhenInspected_IsNotAMenuItemKind()
    {
        ...
        typeof(InputBase).IsAssignableFrom(separator).ShouldBeFalse();
