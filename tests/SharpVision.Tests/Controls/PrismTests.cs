...
public sealed class PrismTests
{
    ...
    /// <summary>Verifies child foregrounds change after rendering without altering rich semantics or Prism chrome.</summary>
    [Fact]
    public void Render_WhenContentIsRichAndBordered_RecolorsOnlyChildForegroundsDeterministically()
    {
        ...
        prism.Bounds.ShouldBe(new Rect(0, 0, 5, 3));
        content.Bounds.ShouldBe(new Rect(1, 1, 3, 1));
        first.GetCell(default).Style.Foreground.ShouldBe(Color.Indexed(7));
        AssertRichStyle(first.GetCell(new Point(1, 1)).Style, Color.Rgb(255, 0, 0));
        AssertRichStyle(first.GetCell(new Point(2, 1)).Style, Color.Rgb(255, 255, 0));
        AssertRichStyle(first.GetCell(new Point(3, 1)).Style, Color.Rgb(0, 255, 0));
        ...
        second.GetCell(default).Style.Foreground.ShouldBe(Color.Indexed(7));
        AssertRichStyle(second.GetCell(new Point(1, 1)).Style, Color.Rgb(0, 255, 255));
    }
    ...
    /// <summary>Verifies Prism post-processing does not recolor its border or unrelated frame cells.</summary>
    [Fact]
    public void Render_WhenFrameHasUnrelatedCells_TransformsOnlyStoredContentInsideContentBounds()
    {
        ...
        var outsideStyle = new TerminalStyle(Color.Indexed(9), Color.Indexed(2));
        _ = frame.Canvas.Draw("Z", new Point(4, 2), outsideStyle);

        prism.Render(frame.Canvas);

        frame.GetCell(default).Style.Foreground.ShouldBe(Color.Indexed(7));
        frame.GetCell(new Point(1, 1)).Style.Foreground.ShouldBe(Color.Rgb(255, 0, 0));
    ...
    /// <summary>Verifies null, empty, and border-consumed content bounds render without an effect failure.</summary>
    [Fact]
    public void Render_WhenContentOrContentBoundsAreEmpty_HasNoForegroundEffect()
    {
        ...
        tiny.Content.Bounds.ShouldBe(new Rect(1, 1, 0, 0));
        tinyFrame.GetCell(default).Style.Foreground.ShouldBe(Color.Indexed(7));
    }
    ...
    private static void AssertRichStyle(TerminalStyle style, Color foreground)
    {
        ...
        style.Background.ShouldBe(Color.Indexed(4));
        ...
        style.UnderlineColor.ShouldBe(Color.Indexed(5));
    }
        ...
        TerminalStyle original)
    {
        ...
        actual.Foreground.Kind.ShouldBe(original.Foreground.Kind);
