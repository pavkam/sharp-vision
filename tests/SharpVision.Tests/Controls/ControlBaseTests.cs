...
public sealed partial class ControlBaseTests
{
    ...
    public void Constructor_WhenCreated_HasDocumentedDefaults()
    {
        ...
        control.HitTestVisible.ShouldBeTrue();
    ...
    {
        var control = new ProbeControl { Bounds = new Rect(0, 0, 2, 1), Focusable = true, HitTestVisible = false };
    ...
    /// <summary>Verifies an ancestor's Enabled change publishes the derived properties it flips
    ...
    public void IsEnabled_WhenAncestorChanges_PublishesDerivedPropertiesOnAffectedDescendants()
    {
        ...
        var alreadyDisabledChild = new ProbeControl { Focusable = true, Enabled = false };
        ...
        parentEvents.ShouldBe([nameof(ControlBase.Enabled), nameof(ControlBase.EffectiveIsEnabled)]);
    ...
    /// <summary>Verifies changing Focusable directly (not through an ancestor's Enabled or
    /// Visibility) also publishes the CanFocus and CanTabStop it flips, matching the ancestor-driven
    /// paths above instead of stopping at Focusable and CanFocus.</summary>
    [Fact]
    ...
    /// <summary>Verifies changing TabStop publishes the CanTabStop it directly flips, since
    /// CanTabStop = CanFocus &amp;&amp; TabStop depends on TabStop just as much as on CanFocus.</summary>
    [Fact]
    public void TabStop_WhenChanged_PublishesIsTabStop()
    {
        ...
        events.ShouldBe([nameof(ControlBase.TabStop), nameof(ControlBase.CanTabStop)]);
    ...
    public async Task IsEnabled_WhenCleanupAndPropertySubscriberThrow_CompletesCleanupAndPreservesFirstFailureAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            child.PropertyChanged += (_, eventArgs) =>
            {
                if (eventArgs.PropertyName != nameof(ControlBase.Enabled))
                {
                    return;
                }
            ...
            var exception = Should.Throw<InvalidOperationException>(() => child.Enabled = false);
    ...
    public async Task IsHitTestVisible_WhenAttachedAndSetOffThread_ThrowsBeforeMutationAsync()
    {
        ...
        _ = Should.Throw<InvalidOperationException>(() => control.HitTestVisible = false);
    ...
    public void Setter_WhenStateIsInvalid_ThrowsDocumentedException()
    {
        ...
        control.Dispose();
        _ = Should.Throw<ObjectDisposedException>(() => control.Enabled = false);
    }
    ...
    /// through the protected PressedState seam, independent of the framework's own pointer/
    /// keyboard press tracking - matching CheckedState, SelectedState, CurrentState, and
    /// IndeterminateState, the four sibling seams that already exist.</summary>
    [Fact]
    public void IsPressedState_WhenOverriddenTrue_DrivesAppearanceStateIndependentlyOfIsPressed()
    {
        var control = new ProbeControl();

        control.Pressed.ShouldBeFalse();
