...
public sealed class ControlCompositeAppearanceTests
{
    ...
    public void ActualAppearance_WhenThemeProvidesPerStateAppearance_UsesNormalAndActiveValues()
    {
        var theme = ThemeCatalog.Parse(ThemeJson.Create(extraStyles:
            $$""", "button": { "normal": { "face": { "foreground": "#010203" }, "border": { "sides": "all", "foreground": "#040506" }, "shadow": { "visible": false } }, "pointerOver": { "face": { "foreground": "#070809" }, "border": { "foreground": "#0a0b0c" }, "shadow": { "background": "#0d0e0f" } } } """));
        var control = new StyledProbe();
        ...
        normalShadowActual.Visible.ShouldBeFalse();
    ...
    public void Style_WhenAppearanceChanges_NotifiesAfterEveryResolvedValueIsCommitted()
    {
        ...
        control.PropertyChanged += (_, eventArgs) =>
        {
            ...
            control.ActualShadow.Visible.ShouldBeTrue();
    ...
    /// <summary>Verifies Enabled routes through the same invalidation-impact decision every other
    ...
    /// <summary>Verifies a descendant that inherits Disabled from an ancestor's Enabled change
    ...
    private static Theme ThemeWithInputFace(Color foreground) => ThemeCatalog.Parse(ThemeJson.Create(
        inputExtra: $$""", "face": { "foreground": "{{Hex(foreground)}}" }"""));
