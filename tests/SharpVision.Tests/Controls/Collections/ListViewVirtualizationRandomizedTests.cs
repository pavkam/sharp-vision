...
/// behavior is already trusted and unaffected by this change (see #231).</summary>
public sealed class ListViewVirtualizationRandomizedTests
{
    ...
    private static void RunCase(int caseIndex, int caseSeed)
    {
        ...
        // currently realized subset's width (see #231's own Extent.Width note below), an accepted
        ...
        int step)
    {
        ...
        // width never depends on which rows happen to be realized (see #231). Neither value
