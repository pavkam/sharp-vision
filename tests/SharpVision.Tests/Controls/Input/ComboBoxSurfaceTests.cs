...
public sealed class ComboBoxSurfaceTests
{
    ...
    public async Task Input_WhenConnectedDropDownRowIsClicked_CommitsChoiceAsync()
    {
        ...
        await using var surface = await ComponentSurface.MountAsync(
            ...
            new Size(12, 7),
            TestContext.Current.CancellationToken);
        await surface.UpdateAsync(() => combo.Opened = true, "open connected ComboBox drop-down");
    ...
    public async Task PopupStyle_WhenSetAndOpen_RendersOverriddenBorderGlyphAsync()
    {
        ...
        // Act
        await surface.UpdateAsync(() => combo.Opened = true, "open drop-down with overridden popup style");
    ...
    public async Task Input_WhenDropDownWheelCannotScroll_StaysOpenWithoutParentScrollAsync()
    {
        ...
        await surface.UpdateAsync(
            ...
            "focus ComboBox");
        await surface.UpdateAsync(() => combo.Opened = true, "open non-scrollable ComboBox");
    ...
    public async Task Input_WhenWheelIsOutsideDropDown_ClosesWithoutParentScrollAsync()
    {
        ...
        await surface.UpdateAsync(
            () => surface.Application.Focus.Focus(combo).ShouldBeTrue(),
            "focus ComboBox");
        await surface.UpdateAsync(() => combo.Opened = true, "open ComboBox");
        var scope = surface.Application.Modality.Active.ShouldNotBeNull();
    ...
    public async Task Input_WhenDropDownListScrolls_KeepsOpenWithoutParentScrollAsync()
    {
        ...
        await surface.UpdateAsync(
            () => surface.Application.Focus.Focus(combo).ShouldBeTrue(),
            "focus ComboBox");
        await surface.UpdateAsync(() => combo.Opened = true, "open scrollable ComboBox");
        var list = OwnedTree.Find<UiListView>(combo).ShouldNotBeNull();
    ...
    public async Task Input_WhenMountedEmpty_ShowsPlaceholderAndTypeAheadSelectsAsync()
    {
        ...
        await surface.UpdateAsync(() => combo.Opened = true, "open ComboBox for type ahead");
    ...
    public async Task Render_WhenDropDownIsOpen_ShowsPopupListWithAllItemsAsync()
    {
        ...
        // Act
        await surface.UpdateAsync(() => combo.Opened = true, "open ComboBox drop-down");
        var list = OwnedTree.Find<UiListView>(combo).ShouldNotBeNull();
    ...
    public async Task Input_WhenDropDownNavigates_CommitsReleasedChoiceAndCleanupAsync()
    {
        ...
        await surface.Pointer.MoveToAsync(combo);
        await surface.Pointer.PressAsync();
        await surface.UpdateAsync(() => combo.Enabled = false, "disable held ComboBox");

        // Assert cleanup
        combo.Pressed.ShouldBeFalse();
        combo.Focused.ShouldBeFalse();
        combo.Opened.ShouldBeFalse();
    ...
    public async Task Input_WhenDefaultHeightDropDownOpens_KeepsFieldHeightAndConnectsListAsync()
    {
        ...
        // Act
        await surface.UpdateAsync(() => combo.Opened = true, "open default-height ComboBox drop-down");
