...
public sealed class ComboBoxTests
{
    ...
    {
        var box = new ComboBox { Items = ["One", "Two", "Three"], DropDownHeight = 4, Opened = true };
    ...
    {
        var box = new ComboBox { DropDownHeight = 4, Items = ["1Row", "3-D", "Standard"], Opened = true };
    ...
    {
        var box = new ComboBox { DropDownHeight = 8, Items = ["One", "Two"], Opened = true };
    ...
    {
        var box = new ComboBox { Items = [new Fruit("Kiwi"), new Fruit("Mango")], SelectedIndex = 1, Opened = true };
        new LayoutEngine().Layout(box, new Size(24, 12));
        box.Opened = false;
        new LayoutEngine().Layout(box, new Size(24, 12));
        var selectionChanges = 0;
    ...
    {
        var box = new ComboBox { Items = ["a", "b", "c"], SelectedIndex = 2, Opened = true };
        new LayoutEngine().Layout(box, new Size(24, 12));
        box.Opened = false;
        new LayoutEngine().Layout(box, new Size(24, 12));
        var selectionChanges = 0;
    ...
    public async Task IsOpen_WhenChanged_PublishesPropertyChangedOnBothTransitionsAsync()
    {
        ...
        await dispatcher.InvokeAsync(() =>
        {
            ...
            box.PropertyChanged += (_, eventArgs) =>
            {
                if (eventArgs.PropertyName == nameof(ComboBox.Opened))
                {
                    notifications.Add(box.Opened);
                }
            };
