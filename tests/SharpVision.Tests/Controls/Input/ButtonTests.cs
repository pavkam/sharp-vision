...
public sealed class ButtonTests
{
    ...
    [Fact]
    public void Style_WhenThemeChanges_UsesSemanticInputProfileWithoutReplacingControlStructure()
    {
        var theme = new Theme();
        ...
        button.Style.ShouldBeNull();
        button.ActualStyle.Padding.ShouldBe(ButtonStyle.Standard.Padding);
        button.ActualFace.ShouldBe(input.Normal.Face);
        button.ActualBorder.ShouldBe(input.Normal.Border);
    ...
    {
        var normal = ButtonStyle.Standard.Appearance.Normal;
        var coloredFace = new Face(
            Color.Rgb(1, 2, 3),
            normal.Face.Background,
            normal.Face.Attributes,
            normal.Face.Underline,
            normal.Face.UnderlineColor);
        var colored = new ButtonStyle(
            ButtonStyle.Standard.Padding,
            new ThemeProfile(new ThemeAppearance(coloredFace, normal.Border, normal.Shadow)));
    ...
    {
        var style = new ButtonStyle(new Thickness(horizontal: 2, vertical: 1), TestButtonStyles.Flat.Appearance);
