...
public sealed class ControlPositionTests
{
    ...
    /// <summary>Verifies SharpVision.Controls.Canvas.SetLeft delegates to Control.Left.</summary>
    [Fact]
    public void CanvasSetLeft_WhenCalled_DelegatesToControlProperty()
    {
        var control = new ProbeControl();

        SharpVision.Controls.Canvas.SetLeft(control, Length.Cells(7));

        control.Left.ShouldBe(Length.Cells(7));
        SharpVision.Controls.Canvas.GetLeft(control).ShouldBe(Length.Cells(7));
    }

    /// <summary>Verifies SharpVision.Controls.Canvas.SetTop delegates to Control.Top.</summary>
    [Fact]
    public void CanvasSetTop_WhenCalled_DelegatesToControlProperty()
    {
        var control = new ProbeControl();

        SharpVision.Controls.Canvas.SetTop(control, Length.Cells(3));

        control.Top.ShouldBe(Length.Cells(3));
        SharpVision.Controls.Canvas.GetTop(control).ShouldBe(Length.Cells(3));
    }

    /// <summary>Verifies Canvas layout reads position from Control properties.</summary>
    [Fact]
    public void CanvasLayout_WhenControlHasPosition_ArrangesCorrectly()
    {
    ...
    {
        var canvas = new SharpVision.Controls.Canvas { Padding = new Thickness(2) };
