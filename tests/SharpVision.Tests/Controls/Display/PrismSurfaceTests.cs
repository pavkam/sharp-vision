...
public sealed class PrismSurfaceTests
{
    ...
    public async Task Enabled_WhenPrismIsDisabledAndReenabled_CascadesStateAndPreservesGeometryAsync()
    {
        ...
        await surface.UpdateAsync(() => prism.IsEnabled = false, "disable Prism");

        // Assert direct and cascaded disabled state
        surface.ShouldHaveState(prism, VisualState.Disabled);
        ...
        var reference = new Prism
        {
            Content = new ControlText("ABCD"),
            Direction = PrismDirection.Horizontal,
            CycleLength = 4,
            HorizontalAlignment = HorizontalAlignment.Stretch,
            VerticalAlignment = VerticalAlignment.Stretch
        ...
        await using var referenceSurface = await ComponentSurface.MountAsync(
            ...
            new Size(6, 2),
            TestContext.Current.CancellationToken);
        prism.Bounds.ShouldBe(reference.Bounds);
        prism.DesiredSize.ShouldBe(reference.DesiredSize);

        // Act re-enable
        await surface.UpdateAsync(() => prism.IsEnabled = true, "re-enable Prism");

        // Assert recovery
        surface.ShouldHaveState(prism, VisualState.Normal);
