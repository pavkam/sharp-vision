...
public sealed class SpinnerSurfaceTests
{
    ...
    public async Task UpdateAsync_WhenOnlyAppearanceChanges_PreservesAdvancedFrameAsync()
    {
        ...
        var firstAppearance = LiteralProfile(Color.Rgb(0x00, 0x00, 0xFF));
        var secondAppearance = LiteralProfile(Color.Rgb(0xFF, 0x00, 0x00));
        var firstStyle = new SpinnerStyle(frames, firstAppearance);
        var secondStyle = new SpinnerStyle(frames, secondAppearance);
