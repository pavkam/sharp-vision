...
public sealed class PrismTests
{
    ...
    /// a complete Face would have made every state identical.</summary>
    [Fact]
    ...
    public void Enabled_WhenToggled_CascadesEffectiveIsEnabledToOwnedContent()
    {
        ...
        prism.IsEnabled = false;

        prism.EffectiveIsEnabled.ShouldBeFalse();
        content.EffectiveIsEnabled.ShouldBeFalse();

        prism.IsEnabled = true;

        prism.EffectiveIsEnabled.ShouldBeTrue();
        content.EffectiveIsEnabled.ShouldBeTrue();
    }
