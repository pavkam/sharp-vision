...
public sealed class AnimationTimerTests
{
    ...
    public void Constructor_WhenCreated_StoresIntervalAndDoesNotPlay()
    {
        ...
        timer.Playing.ShouldBeFalse();
    }

    /// <summary>Verifies Attach creates and starts the timer when Playing is true.</summary>
    [Fact]
    ...
    /// <summary>Verifies Attach does not start the timer when Playing is false.</summary>
    [Fact]
    ...
    public async Task Detach_WhenAttached_ReleasesTimerAndStopsTicksAsync()
    {
        ...
        await using var dispatcher = Dispatcher.Start(timeProvider: clock);
        var ticks = 0;
        var timer = new AnimationTimer(TimeSpan.FromMilliseconds(100), () => ticks++) { Playing = true };
        await dispatcher.InvokeAsync(
            () => timer.Attach(dispatcher),
            TestContext.Current.CancellationToken);
    ...
    /// <summary>Verifies Playing starts the timer after attach.</summary>
    [Fact]
    ...
    /// <summary>Verifies Playing stops the timer.</summary>
    [Fact]
