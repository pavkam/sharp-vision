...
public sealed class NavigationViewSurfaceTests
{
    ...
    public async Task Render_WhenEverySectionIsPresent_DrawsExactRetainedSidebarAsync()
    {
        ...
        mainSeparator.PointerOver.ShouldBeFalse();
        mainSeparator.Focused.ShouldBeFalse();
        mainSeparator.Pressed.ShouldBeFalse();
    }
    ...
    public async Task Input_WhenItemsNavigate_UsesOneFlatEligibleSelectionOrderAsync()
    {
        ...
        var disabled = new NavigationViewItem { Text = "Disabled", Enabled = false };
        ...
        first.Pressed.ShouldBeFalse();

        // Act unavailable while another item press is held
        await surface.Pointer.PressAsync();
        await surface.UpdateAsync(() => first.Enabled = false, "disable held NavigationViewItem");
        ...
        await surface.UpdateAsync(() => view.Enabled = false, "disable focused NavigationView");
    ...
    public async Task Pointer_WhenSecondGroupHeaderIsClicked_TogglesThatGroupIndependentlyAsync()
    {
        ...
        await using var surface = await ComponentSurface.MountAsync(
            ...
            new Size(16, 6),
            TestContext.Current.CancellationToken);
        first.Expanded.ShouldBeTrue();
        second.Expanded.ShouldBeTrue();
    ...
    public async Task Input_WhenGroupsReceiveKeyboardNavigation_CollapsesEachGroupAsync()
    {
        ...
        // Assert
        core.Expanded.ShouldBeFalse();
        tests.Expanded.ShouldBeFalse();
