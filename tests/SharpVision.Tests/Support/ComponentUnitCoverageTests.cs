...
public sealed class ComponentUnitCoverageTests
{
    ...
    // key, so a catalog mistake fails the moment this type initializes.
    private static readonly Dictionary<Type, Type> _fixtures = new()
    {
        { typeof(ControlText), typeof(TextTests) },
        { typeof(HorizontalBarChart), typeof(ChartControlTests) },
        { typeof(VerticalBarChart), typeof(ChartControlTests) },
        { typeof(LineChart), typeof(ChartControlTests) },
        { typeof(AreaChart), typeof(ChartControlTests) },
        { typeof(Sparkline), typeof(ChartControlTests) },
        ...
        { typeof(NumberInput), typeof(NumberInputTests) },
        { typeof(CurrencyInput), typeof(CurrencyInputTests) },
        { typeof(ColorPicker), typeof(ColorPickerTests) },
        { typeof(JsonView), typeof(JsonViewTests) },
        { typeof(UiListView), typeof(ListViewTests) },
    ...
    /// rather than merely containing some unrelated executable test.</summary>
    [Fact]
    ...
    /// <summary>Verifies every control classified Disabled in the mounted map also has a detached
    /// unit test proving the same disabled contract, so the mounted evidence is not the only place
    /// IsEnabled=false is ever honestly exercised.</summary>
    [Fact]
    public void Fixtures_WhenControlRequiresMountedDisabledEvidence_ContainDisabledUnitEvidence()
    {
        foreach (var (control, requirement) in ComponentSurfaceCoverageTests.Requirements)
        {
            if ((requirement.Behaviors & ComponentBehavior.Disabled) == 0)
            {
                continue;
            }

            var fixture = _fixtures[control];
            var hasDisabledEvidence = fixture.GetMethods()
                .SelectMany(method => method.GetCustomAttributes(inherit: false))
                .OfType<ComponentUnitEvidenceAttribute>()
                .Any(evidence =>
                    evidence.ControlType == control &&
                    (evidence.Behaviors & ComponentBehavior.Disabled) != 0);

            hasDisabledEvidence.ShouldBeTrue(
                $"{control.Name} fixture {fixture.Name} must declare " +
                $"[ComponentUnitEvidence(typeof({control.Name}), ComponentBehavior.Disabled)] " +
                "on at least one detached unit test.");
        }
    }
}
