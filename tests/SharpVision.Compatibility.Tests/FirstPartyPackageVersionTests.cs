...
///
/// <para><c>SharpVision.FigletFonts</c> consumes <c>SharpVision</c> as a package rather than a
/// project reference, so the dependency floor comes from <c>Directory.Packages.props</c> like any
/// third-party dependency. NuGet resolves a floor range to the <em>lowest</em> satisfying version
/// across all sources, and nuget.org carries every previously published core - so a literal floor
/// that falls behind <c>SharpVisionVersion</c> does not fail, it silently builds, packs, and
/// consumer-tests the font package against an older core while the freshly packed one sits unused
/// in the bootstrap feed. That is what happened before each of the three libraries owned an
/// independent version: the floor was set equal to the then-current version when the package was
/// split out, the shared version moved on three times, and the floor was never revisited.</para>
///
...
public sealed class FirstPartyPackageVersionTests
{
    ...
    /// version from the specific property the publish workflow reads for that dependency, so a
    /// literal cannot be reintroduced and then quietly go stale.</summary>
    [Fact]
    public void PackageVersions_WhenFirstParty_DeriveFromSharpVisionVersion()
    {
        ...
        {
            version.Contains("$(SharpVisionVersion)", StringComparison.Ordinal).ShouldBeTrue(
    ...
    [Fact]
    public void PackageVersions_WhenDerived_ExpandToTheCurrentSharpVisionVersion()
    {
        var sharpVisionVersion = SharpVisionVersion();

        sharpVisionVersion.ShouldNotBeNullOrWhiteSpace();

        foreach (var (package, version) in FirstPartyPackageVersions())
        {
            var expanded = version.Replace("$(SharpVisionVersion)", sharpVisionVersion, StringComparison.Ordinal);
            ...
            expanded.Contains(sharpVisionVersion, StringComparison.Ordinal).ShouldBeTrue(
                $"'{package}' expands to '{expanded}', which must resolve to the SharpVision version " +
                $"being built ('{sharpVisionVersion}') rather than an older published one");
        }
    }
    ...
    private static string SharpVisionVersion()
    {
        var document = XDocument.Load(Path.Combine(FindRepositoryRoot(), "Directory.Build.props"));

        return document.Descendants("SharpVisionVersion").FirstOrDefault()?.Value.Trim() ?? string.Empty;
    }
