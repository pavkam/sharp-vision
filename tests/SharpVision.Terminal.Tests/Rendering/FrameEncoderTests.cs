...
public sealed class FrameEncoderTests
{
    ...
    /// <summary>Verifies underline color is withheld, and no invalid style is constructed, when the only
    /// underline signal (legacy "smul") cannot be emitted.</summary>
    [Fact]
    public void Encode_WhenUnderlineCannotSurviveProjection_WithholdsUnderlineColorAndDoesNotThrow()
    {
        using Frame frame = new(new Size(1, 1));
        _ = frame.Canvas.Draw(
            "x",
            ...
            new CellStyle(
                attributes: TerminalAttributes.Underline,
                underlineColor: ReferenceColors.Get(9)));
        var programs = CorePrograms();
        programs["Setulc"] = new DescriptionProgram("P%p1%d"u8);
        var supported = new Feature(CapabilitySupport.Supported, Origin.Override);
        var profile = new TerminalProfile(
            new Description("underline-orphan", DescriptionOrigin.Database, Suitability.Usable, colors: 256),
            TerminalCapabilities.Conservative with
            {
            ...
            KeyMap.Empty);
        var destination = new ArrayBufferWriter<byte>();
        var resolved = ReferenceColors.Get(9);
        var packed = (resolved.Red << 16) | (resolved.Green << 8) | resolved.Blue;

        var exception = Record.Exception(() => FrameEncoder.Encode(null, frame, destination, profile));

        exception.ShouldBeNull();
        destination.WrittenSpan.IndexOf(Encoding.ASCII.GetBytes($"P{packed}")).ShouldBe(-1);
    }
