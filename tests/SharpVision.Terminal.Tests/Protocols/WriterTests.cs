...
public sealed class WriterTests
{
    ...
    public void Write_WhenGrammarIsInvalid_ThrowsBeforeWriting()
    {
        ...
        _ = Should.Throw<ArgumentException>(() => writer.Csi("A"u8, [], (byte) 'H'));
        _ = Should.Throw<ArgumentException>(() => writer.Csi([], "0"u8, (byte) 'H'));
        _ = Should.Throw<ArgumentOutOfRangeException>(() => writer.Csi([], [], 0x3f));
        _ = Should.Throw<ArgumentException>(() => writer.Osc(2, [(byte) 'a', 0x1b, (byte) 'b']));
        _ = Should.Throw<ArgumentOutOfRangeException>(() => writer.Osc(-1, []));
        _ = Should.Throw<ArgumentOutOfRangeException>(() => writer.Command(SequenceKind.Csi, []));
        _ = Should.Throw<ArgumentException>(() => writer.Dcs([], [], (byte) 'q', [(byte) 'a', 0x07]));
