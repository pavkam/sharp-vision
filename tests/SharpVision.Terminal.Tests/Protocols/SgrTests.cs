...
public sealed class SgrTests
{
    ...
    /// <summary>
    /// Verifies default and RGB foreground encoding.
    /// </summary>
    [Fact]
    public void Foreground_WhenColorIsValid_WritesExactBytes()
    {
        ...
        destination.WrittenSpan.ToArray().ShouldBe(
            "\u001b[39m\u001b[38;2;1;2;3m"u8.ToArray());
    }

    /// <summary>
    /// Verifies default and RGB background encoding.
    /// </summary>
    [Fact]
    public void Background_WhenColorIsValid_WritesExactBytes()
    {
        ...
        destination.WrittenSpan.ToArray().ShouldBe(
            "\u001b[49m\u001b[48;2;254;253;252m"u8.ToArray());
    }
    ...
    /// <summary>Verifies default and RGB underline-color output.</summary>
    [Fact]
    public void UnderlineColor_WhenColorIsValid_WritesExactBytes()
    {
        ...
        destination.WrittenSpan.ToArray().ShouldBe(
            "\u001b[59m\u001b[58;2;1;2;3m"u8.ToArray());
    }

    /// <summary>Verifies internal palette positions remain exact bounded protocol output.</summary>
    [Fact]
    public void PaletteColor_WhenPositionIsValid_WritesExactBytes()
    {
        ...
        Sgr.UnderlineColorPalette(writer, 67);

        destination.WrittenSpan.ToArray().ShouldBe(
            "\u001b[38;5;123m\u001b[48;5;255m\u001b[58;5;67m"u8.ToArray());
    }
    ...
    public void Command_WhenValueIsInvalid_ThrowsBeforeWriting()
    {
        ...
        _ = Should.Throw<ArgumentOutOfRangeException>(() => Sgr.Foreground(writer, (BasicColor) 999));
        _ = Should.Throw<ArgumentOutOfRangeException>(() => Sgr.Background(writer, (BasicColor) 999));
        _ = Should.Throw<ArgumentOutOfRangeException>(() => Sgr.Apply(writer, (Underline) 999));
        _ = Should.Throw<ArgumentOutOfRangeException>(() => Sgr.ForegroundPalette(writer, -1));
        _ = Should.Throw<ArgumentOutOfRangeException>(() => Sgr.BackgroundPalette(writer, 256));

        destination.WrittenCount.ShouldBe(0);
    }

    /// <summary>Verifies unresolved and transparent colors never become terminal reset sequences.</summary>
    [Fact]
    public void ColorCommand_WhenColorIsNotConcrete_ThrowsBeforeWriting()
    {
        ...
        _ = Should.Throw<ArgumentException>(() => Sgr.UnderlineColor(writer, Color.Transparent));
