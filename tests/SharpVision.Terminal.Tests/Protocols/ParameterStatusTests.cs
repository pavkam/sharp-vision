...
/// </summary>
public sealed class ParameterStatusTests
{
    /// <summary>
    ...
    /// <summary>
    /// Verifies a stray non-decimal byte encountered after a field has already overflowed does
    /// not downgrade the more specific Overflow diagnosis back to a bare Invalid.
    /// </summary>
    [Fact]
    public void Read_WhenOverflowedFieldAlsoContainsInvalidByte_StaysOverflow()
    {
        var parameters = new Parameters("99999999999x"u8, maxCount: 8, maxValue: 1000);
    ...
    {
        _ = Should.Throw<ArgumentOutOfRangeException>(static () => _ = new Parameters([], maxCount: 0, maxValue: 1));
        _ = Should.Throw<ArgumentOutOfRangeException>(static () => _ = new Parameters([], maxCount: 1, maxValue: 0));
    }
