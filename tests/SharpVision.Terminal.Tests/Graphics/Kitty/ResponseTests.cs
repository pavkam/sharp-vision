...
public sealed class ResponseTests
{
    ...
    /// <summary>
    /// Verifies a placement-only acknowledgement (no image id or number field) is grammar-valid,
    /// since a placement-affecting request such as an animation frame update legitimately echoes
    /// only the placement id.
    /// </summary>
    [Fact]
    public void Parse_WhenReplyIsPlacementOnly_AcceptsResponse()
    {
        var response = KittyGraphicsResponse.Parse("Gp=2;OK"u8);

        response.Valid.ShouldBeTrue();
        response.ImageId.ShouldBe(0U);
        response.PlacementId.ShouldBe(2U);
        response.ImageNumber.ShouldBe(0U);
        response.Succeeded.ShouldBeTrue();
    }
    ...
    /// the two concepts are independently configurable now that they have been split apart,
    /// closing the silent cross-domain leak.</summary>
    [Fact]
    ...
    public void Parse_WhenMalformedReplyPrecedesValidReply_RecoversAtNextApc()
    {
        ...
        for (var split = 0; split <= wire.Length; split++)
        {
            ...
            sink.KittyGraphicsResponses[0].Valid.ShouldBeFalse();
            sink.KittyGraphicsResponses[1].ShouldSatisfyAllConditions(
                response => response.Valid.ShouldBeTrue(),
