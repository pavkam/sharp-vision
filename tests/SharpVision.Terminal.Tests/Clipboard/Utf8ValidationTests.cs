...
public sealed class Utf8ValidationTests
{
    ...
    [Fact]
    public void IsValid_WhenSpanIsEmpty_ReturnsTrue() => ReadOnlySpan<byte>.Empty.Valid().ShouldBeTrue();

    /// <summary>Verifies plain ASCII bytes are valid.</summary>
    [Fact]
    public void IsValid_WhenBytesAreAscii_ReturnsTrue() => "hello"u8.Valid().ShouldBeTrue();
    ...
    public void IsValid_WhenBytesContainALoneContinuationByte_ReturnsFalse() =>
        ((ReadOnlySpan<byte>) [0x80]).Valid().ShouldBeFalse();
    ...
    public void IsValid_WhenBytesEndInATruncatedSequence_ReturnsFalse() =>
        ((ReadOnlySpan<byte>) [0xE2, 0x82]).Valid().ShouldBeFalse();
    ...
    public void IsValid_WhenBytesContainAnInvalidLeadByte_ReturnsFalse() =>
        ((ReadOnlySpan<byte>) [0xFF]).Valid().ShouldBeFalse();
}
