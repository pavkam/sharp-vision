...
public sealed class WidthTests
{
    ...
    {
        var result = Width.Measure("A界👩‍💻".AsSpan());
    ...
    {
        Width.Measure("·".AsSpan()).Cells.ShouldBe(1);
    ...
    {
        Width.Measure("☀︎".AsSpan()).Cells.ShouldBe(1);
        Width.Measure("☀️".AsSpan()).Cells.ShouldBe(2);
    }
    ...
    public void Measure_WhenClusterIsEmojiSequence_ReturnsTwoCells(string value) =>
        Width.Measure(value.AsSpan()).Cells.ShouldBe(2);
    ...
    /// </summary>
    [Fact]
    public void Measure_WhenAmbiguousPolicyIsUnknown_ThrowsArgumentOutOfRangeException() => _ = Should.Throw<ArgumentOutOfRangeException>(() => Width.Measure("x".AsSpan(), (Ambiguous) 99));
}
