...
public sealed class InputDecoderTests
{
    ...
    [InlineData("\u001b[<28;10;5M", Buttons.Primary, InputAction.Press, 0, 0,
        Modifiers.Shift | Modifiers.Meta | Modifiers.Control, false)]
    ...
    [InlineData("\u001b[1;9A", Code.Up, Modifiers.Meta, 0)]
    [InlineData("\u001b[1;10A", Code.Up, Modifiers.Meta | Modifiers.Shift, 0)]
    [InlineData("\u001b[1;11A", Code.Up, Modifiers.Meta | Modifiers.Alt, 0)]
    [InlineData("\u001b[1;12A", Code.Up, Modifiers.Meta | Modifiers.Alt | Modifiers.Shift, 0)]
    [InlineData("\u001b[1;13A", Code.Up, Modifiers.Meta | Modifiers.Control, 0)]
    [InlineData("\u001b[1;14A", Code.Up, Modifiers.Meta | Modifiers.Control | Modifiers.Shift, 0)]
    [InlineData("\u001b[1;15A", Code.Up, Modifiers.Meta | Modifiers.Control | Modifiers.Alt, 0)]
    [InlineData("\u001b[1;16A", Code.Up, Modifiers.Meta | Modifiers.Control | Modifiers.Alt | Modifiers.Shift, 0)]
    ...
    #region Ground-state text-run batching

    /// <summary>
    /// Verifies a ground-state text run immediately followed by bracketed paste markers decodes
    /// identically to the byte-at-a-time path at every transport split: the batched pre-scan must
    /// stop before the paste-begin marker's leading Escape rather than folding it into the text
    /// run.
    /// </summary>
    [Fact]
    public void Decode_WhenGroundTextRunPrecedesBracketedPaste_BatchesTextAndPreservesPaste()
    {
        var bytes = Encoding.UTF8.GetBytes("hello\u001b[200~pasted content\u001b[201~world");
        ...
        for (var split = 0; split <= bytes.Length; split++)
        {
            ...
            sink.Text.Select(static item => item.Value.Value)
                .ShouldBe("helloworld".Select(static ch => (int) ch), $"split {split}");
            Encoding.UTF8.GetString(sink.Pastes.Single().Utf8.Span)
                .ShouldBe("pasted content", $"split {split}");
            sink.Diagnostics.ShouldBeEmpty($"split {split}");
        }
    }

    /// <summary>
    /// Verifies a ground-state text run immediately followed by an SS3 arrow key decodes
    /// identically to the byte-at-a-time path at every transport split: the batched pre-scan must
    /// stop at the arrow key's leading Escape.
    /// </summary>
    [Fact]
    public void Decode_WhenGroundTextRunPrecedesSs3ArrowKey_BatchesTextAndPreservesArrowKey()
    {
        var bytes = Encoding.UTF8.GetBytes("hi\u001bOAbye");
        ...
        for (var split = 0; split <= bytes.Length; split++)
        {
            ...
            sink.Strokes.Select(static item => item.Code)
                .ShouldBe(
                    [Code.Character, Code.Character, Code.Up, Code.Character, Code.Character, Code.Character],
                    $"split {split}");
            sink.Text.Select(static item => item.Value.Value)
                .ShouldBe("hibye".Select(static ch => (int) ch), $"split {split}");
            sink.Diagnostics.ShouldBeEmpty($"split {split}");
        }
    }

    /// <summary>
    /// Verifies a ground-state text run surrounding a fallback-matcher-eligible byte still decodes
    /// correctly at every transport split when a <see cref="KeyMap"/> with fallback bindings is
    /// configured: an active fallback matcher must disable the batched pre-scan entirely, since
    /// batching text through a configured matcher would bypass fallback key-sequence recognition.
    /// </summary>
    [Fact]
    public void Decode_WhenGroundTextRunSurroundsFallbackKey_DoesNotBatchThroughFallbackMatcher()
    {
        var options = InputOptions.Default.WithKeyMap(
            new KeyMap([new KeyBinding([0xff], Code.F62)]),
            useAnsiKeyGrammar: false);
        byte[] bytes = [(byte) 'h', (byte) 'i', 0xff, (byte) 'b', (byte) 'y', (byte) 'e'];
        ...
        for (var split = 0; split <= bytes.Length; split++)
        {
            ...
            sink.Strokes.Select(static item => item.Code)
                .ShouldBe(
                    [Code.Character, Code.Character, Code.F62, Code.Character, Code.Character, Code.Character],
                    $"split {split}");
            sink.Text.Select(static item => item.Value.Value)
                .ShouldBe("hibye".Select(static ch => (int) ch), $"split {split}");
            sink.Diagnostics.ShouldBeEmpty($"split {split}");
    ...
    /// <summary>
    /// Verifies a ground-state text run surrounding a byte that a described key map requires as a
    /// structural eight-bit SS3 introducer is never folded into the batched text run: naive
    /// batching that ignored <c>RequiresEightBitSs3</c> would silently swallow 0x8f as ordinary
    /// text instead of letting <see cref="InputDecoder"/> intercept it one byte at a time.
    /// </summary>
    [Fact]
    public void Decode_WhenGroundTextRunSurroundsRequiredEightBitSs3Byte_DoesNotSwallowAsText()
    {
        var options = InputOptions.Default.WithKeyMap(
            new KeyMap([new KeyBinding([0x8f, (byte) 'A'], Code.Up)]),
            useAnsiKeyGrammar: false);
        byte[] bytes = [(byte) 'h', (byte) 'i', 0x8f, (byte) 'A', (byte) 'x'];
        ...
        for (var split = 0; split <= bytes.Length; split++)
        {
            ...
            sink.Strokes.Select(static item => item.Code)
                .ShouldBe(
                    [Code.Character, Code.Character, Code.Up, Code.Character],
                    $"split {split}");
            sink.Text.Select(static item => item.Value.Value)
                .ShouldBe("hix".Select(static ch => (int) ch), $"split {split}");
            sink.Diagnostics.ShouldBeEmpty($"split {split}");
        }
    }
    ...
    /// <summary>
    /// Verifies a ground-state text run surrounding a byte that a described key map requires as a
    /// structural eight-bit CSI introducer is never folded into the batched text run: naive
    /// batching that ignored <c>RequiresEightBitCsi</c> would silently swallow 0x9b as ordinary
    /// text instead of letting <see cref="InputDecoder"/> intercept it one byte at a time.
    /// </summary>
    [Fact]
    public void Decode_WhenGroundTextRunSurroundsRequiredEightBitCsiByte_DoesNotSwallowAsText()
    {
        var options = InputOptions.Default.WithKeyMap(
            new KeyMap([new KeyBinding([0x9b, (byte) '9', (byte) '2', (byte) '~'], Code.F62)]),
            useAnsiKeyGrammar: false);
        byte[] bytes =
        [
            (byte) 'h', (byte) 'i', 0x9b, (byte) '9', (byte) '2', (byte) '~', (byte) 'x'
        ];
        ...
        for (var split = 0; split <= bytes.Length; split++)
        {
            ...
            sink.Strokes.Select(static item => item.Code)
                .ShouldBe(
                    [Code.Character, Code.Character, Code.F62, Code.Character],
                    $"split {split}");
            sink.Text.Select(static item => item.Value.Value)
                .ShouldBe("hix".Select(static ch => (int) ch), $"split {split}");
            sink.Diagnostics.ShouldBeEmpty($"split {split}");
