...
/// <summary>Verifies the bracketed-paste byte accumulator extracted from InputDecoder.</summary>
public sealed class PasteAccumulatorTests
{
    ...
    /// silently truncating the buffer mid-cluster. The count must include the bytes already
    /// buffered when the limit was reached, not just the bytes that arrive afterward - the
    /// buffered bytes are discarded too when the accumulator clears itself into overflow.</summary>
    [Fact]
    public void Process_WhenByteLimitIsReached_OverflowsAndCountsDiscardedBytes()
    {
        ...
        accumulator.DiscardedBytes.ShouldBe(4);
    ...
    public void Reset_WhenPasteWasBuffered_ClearsStateAndKeepsAcceptingFurtherPastes()
    {
        ...
        accumulator.Active.ShouldBeFalse();
        accumulator.Overflowed.ShouldBeFalse();
        accumulator.DiscardedBytes.ShouldBe(0);
        accumulator.Buffered.Length.ShouldBe(0);

        accumulator.Begin();
        accumulator.Active.ShouldBeTrue();
