...
public sealed class QueryTrackerTests
{
    ...
    /// <summary>
    /// Verifies a negative XTGETTCAP reply that echoes the single unsupported capability name
    /// still retires the pending query, since that name identifies which query it answers
    /// even though the terminal did not support it.
    /// </summary>
    [Fact]
    public void Match_WhenFailedResponseEchoesTheRequestedName_RetiresActiveQuery()
    {
        var tracker = new QueryTracker();
        tracker.TryRegister(CapabilityName.DirectColor, out _).ShouldBeTrue();
        var failed = Capability("524742"u8, valid: false);

        tracker.Match(failed).ShouldBe(QueryMatch.Matched);
