...
public sealed class NcursesProviderTests
{
    ...
    /// path per request without recompiling.</summary>
    [Fact]
    ...
    /// so refusing to unload would leak the handle for no safety benefit.</summary>
    [Fact]
    ...
    public void Load_WhenOptionalCommandSetsAreComplete_RecordsDatabaseSupport()
    {
        ...
        _ = result.Profile.ShouldNotBeNull();
        result.Profile.Capabilities.FocusReporting.Supported.ShouldBeTrue();
        result.Profile.Capabilities.BracketedPaste.Supported.ShouldBeTrue();
        result.Profile.Capabilities.CellMouse.Supported.ShouldBeTrue();
    }
    ...
    public void Probe_WhenTerminfoDirectoriesContainSameName_UsesFirstDirectory()
    {
        ...
        {
            (first.Available ? second.Availability : first.Availability).ShouldBe("ToolUnavailable");
            return;
        }
