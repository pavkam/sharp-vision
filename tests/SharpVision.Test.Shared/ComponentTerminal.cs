...
public sealed class ComponentTerminal: ITransport, IResizeSource
{
    ...
    public ComponentTerminal(Size size) => _screen = new ComponentScreen(size);

    /// <summary>Gets the independently modeled terminal screen.</summary>
    public ComponentScreen Screen => Volatile.Read(ref _screen);
    ...
    public Task QueueInput(ReadOnlySpan<byte> value)
    {
        if (value.IsEmpty)
        {
    ...
    /// <exception cref="ObjectDisposedException">The terminal is disposed.</exception>
    public Task QueueResize(Dimensions value)
    {
