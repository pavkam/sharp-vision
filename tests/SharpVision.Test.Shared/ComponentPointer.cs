...
public sealed class ComponentPointer
{
    ...
    public ComponentPointer(ComponentSurface surface)
    {
    ...
    public async Task MoveToAsync(ControlBase control)
    {
    ...
    public async Task MoveToAsync(ControlBase control, Point relative)
    {
    ...
    /// <summary>Moves the pointer to one absolute surface cell, bypassing control-relative bounds
    /// validation. Use this to address cells a control paints outside its own arranged Bounds -
    /// such as a shadow-translated pressed face - which the control-relative overloads cannot
    /// reach.</summary>
    /// <param name="point">The zero-based absolute surface cell.</param>
    /// <returns>A task completed after hover and rendering settle.</returns>
    ...
    /// <exception cref="ArgumentException"><paramref name="control"/> is not owned by the surface.</exception>
    /// <exception cref="InvalidOperationException"><paramref name="control"/> has empty arranged bounds.</exception>
    public async Task ClickAsync(ControlBase control)
    {
    ...
    /// <exception cref="InvalidOperationException"><paramref name="control"/> has empty arranged bounds.</exception>
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="relative"/> lies outside the control.</exception>
    public async Task ClickAsync(ControlBase control, Point relative)
    {
    ...
    public async Task ClickAsync(ControlBase control, Modifiers modifiers)
    {
    ...
    public async Task MovePressedToAsync(ControlBase control, Point relative)
    {
        if (!_primaryPressed)
        {
    ...
    /// <summary>Drags the held primary pointer button to one absolute surface cell, bypassing
    /// control-relative bounds validation. Use this to address cells a control paints outside its
    /// own arranged Bounds - such as a shadow-translated pressed face - which the control-relative
    /// overload cannot reach.</summary>
    /// <param name="point">The zero-based absolute surface cell.</param>
    /// <returns>A task completed after motion, activation, and rendering settle.</returns>
    /// <exception cref="InvalidOperationException">The primary pointer button is not held.</exception>
    public async Task MovePressedToAsync(Point point)
    {
        if (!_primaryPressed)
        {
            throw new InvalidOperationException("Press the primary pointer button before moving it as held.");
    ...
    public async Task DragAsync(ControlBase control, Point start, Point end)
    {
    ...
    /// <summary>Moves to and right-clicks (secondary button) the current arranged center of an owned control.</summary>
    /// <param name="control">The mounted control or one of its owned descendants.</param>
    /// <returns>A task completed after move, secondary press, release, and rendering settle.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="control"/> is null.</exception>
    /// <exception cref="ArgumentException"><paramref name="control"/> is not owned by the surface.</exception>
    /// <exception cref="InvalidOperationException"><paramref name="control"/> has empty arranged bounds.</exception>
    public async Task RightClickAsync(ControlBase control)
    {
        await MoveToAsync(control);
        var point = _lastPoint ?? throw new InvalidOperationException(
            "Move the component pointer before pressing a button.");
        await _surface.SendAsync(Encode(button: 2, point, final: 'M'), $"press secondary pointer at {point}");
        await _surface.SendAsync(Encode(button: 2, point, final: 'm'), $"release secondary pointer at {point}");
    }

    /// <summary>Right-clicks at one cell relative to an owned control's arranged bounds.</summary>
    /// <param name="control">The mounted control or one of its owned descendants.</param>
    /// <param name="relative">The zero-based cell inside the control.</param>
    /// <returns>A task completed after move, secondary press, release, and rendering settle.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="control"/> is null.</exception>
    ...
    /// <exception cref="InvalidOperationException"><paramref name="control"/> has empty arranged bounds.</exception>
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="relative"/> lies outside the control.</exception>
    public async Task RightClickAsync(ControlBase control, Point relative)
    {
        await MoveToAsync(control, relative);
        var point = _lastPoint ?? throw new InvalidOperationException(
            "Move the component pointer before pressing a button.");
