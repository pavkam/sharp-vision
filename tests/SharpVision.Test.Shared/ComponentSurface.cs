...
public sealed class ComponentSurface: IAsyncDisposable
{
    private const VisualState _observableStates = VisualState.IsPointerOver |
                                                  VisualState.Focused |
                                                  VisualState.Pressed |
    ...
    public static async Task<ComponentSurface> MountScreenAsync(
        Screen screen,
        Size size,
        CancellationToken cancellationToken) =>
        ...
        CancellationToken cancellationToken)
    {
        ...
        ObjectDisposedException.ThrowIf(screen.IsDisposed, screen);
    ...
    public SurfaceCell Cell(Point point) => _terminal.Screen.Cell(point);
    ...
    public void ShouldHaveCursor(Point position, bool visible)
    {
    ...
    public void ShouldHaveCursor(Point position, bool visible, CursorShape shape)
    {
    ...
    public async Task<Point> ResolvePointAsync(ControlBase control)
    {
        ArgumentNullException.ThrowIfNull(control);
    ...
    public async Task<Point> ResolvePointAsync(ControlBase control, Point relative)
    {
        ArgumentNullException.ThrowIfNull(control);
        return await Application.Dispatcher.InvokeAsync(() =>
        {
    ...
    /// <param name="control">The mounted control or one of its owned descendants.</param>
    /// <param name="expected">The expected Normal, Hovered, IsFocused, IsPressed, or Disabled flags.</param>
    /// <exception cref="ArgumentNullException"><paramref name="control"/> is null.</exception>
    ...
    public void ShouldHaveState(ControlBase control, VisualState expected)
    {
        ArgumentNullException.ThrowIfNull(control);
        ...
        {
            actual |= VisualState.IsPointerOver;
        }
    ...
    public void ShouldHaveFocus(ControlBase? expected)
    {
    ...
    /// <exception cref="ArgumentException"><paramref name="expected"/> is not owned by this surface.</exception>
    public void ShouldHaveCapture(ControlBase? expected)
    {
        if (expected is not null && !IsOwned(expected))
        {
    ...
    public void ShouldRender(string expected)
    {
