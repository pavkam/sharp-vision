...
description:
  Use when changing SharpVision ANSI, ECMA-48, CSI, OSC, DCS, DEC, xterm, Kitty,
  iTerm2, sixel, tmux, GNU screen, keyboard, mouse, paste, focus, clipboard, or
  terminal capability protocol behavior.
---
...
Keep protocol behavior sourced, typed, bounded, byte-exact, and recoverable. The
normative docs own wire semantics; code and tests prove the declared coverage
state.
...
6. Update the protocol spec and coverage matrix in the same change. Never mark a
   feature implemented until typed behavior and tests exist.
...
- Keep controls protocol-free; terminal bytes belong in `SharpVision.Terminal`.
