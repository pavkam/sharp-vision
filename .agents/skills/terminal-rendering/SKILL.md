...
description:
  Use when changing SharpVision cells, canvases, frame buffers, damage tracking,
  frame scheduling, cursor or SGR emission, synchronized output, terminal
  writes, resize invalidation, or render performance.
---
...
1. Read `docs/architecture/rendering-pipeline.md`,
   ...
   `docs/concepts/unicode-cell-geometry.md`, and `docs/testing/rendering.md`.
...
- The encoder leaves cursor position, cursor visibility, styles, hyperlinks, and
  synchronized-output mode in a known state.
...
For a wide emoji replaced by a narrow letter, require damage to include the lead
and continuation cells, clear stale width, apply the emitted diff to the virtual
terminal, and compare it with a clean full render at the same size.
